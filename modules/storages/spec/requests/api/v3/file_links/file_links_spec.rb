...
describe 'API v3 file links resource', type: :request do
  ...
  describe 'POST /api/v3/work_packages/:work_package_id/file_links' do
    ...
    context "when more than #{API::V3::Utilities::ParseCreateParamsService::MAX_ELEMENTS} embedded elements" do
      let(:max) { API::V3::Utilities::ParseCreateParamsService::MAX_ELEMENTS }
      let(:too_many) { max + 1 }
  ...
  describe 'GET /api/v3/file_links/:file_link_id/download' do
    let(:connection_manager) { instance_double(::OAuthClients::ConnectionManager) }
    let(:request_factory) { instance_double(::API::V3::Utilities::StorageRequests) }
    ...
    before do
      ...
      allow(::API::V3::Utilities::StorageRequests).to receive(:new).and_return(request_factory)

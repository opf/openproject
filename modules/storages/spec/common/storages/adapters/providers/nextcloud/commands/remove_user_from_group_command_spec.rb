          ...
          RSpec.describe RemoveUserFromGroupCommand, :disable_ssrf_filter, :webmock do
            ...
            describe "basic command setup" do
              it "is registered as commands.remove_user_from_group" do
                expect(Registry.resolve("#{storage}.commands.remove_user_from_group")).to eq(described_class)
              end
            ...
            def add_user_to_group(user, group)
              Input::AddUserToGroup.build(user:, group:).bind do |input_data|
                AddUserToGroupCommand.call(storage:, auth_strategy:, input_data:)
              end
            end
            ...
            def create_group(authentication, storage, group)
              authentication.call(storage:, http_options: { headers: { "OCS-APIRequest" => "true" } }) do |http|
                http.post(UrlBuilder.url(storage.uri, "ocs/v1.php/cloud/groups"), form: { "groupid" => group })
              end
            end

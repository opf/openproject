          ...
          RSpec.describe AddUserToGroupCommand, :disable_ssrf_filter, :webmock do
            ...
            describe "basic command setup" do
              it "is registered as commands.add_user_to_group" do
                expect(Registry.resolve("#{storage}.commands.add_user_to_group")).to eq(described_class)
              end
            ...
            def group_members(group)
              Input::GroupUsers.build(group:).bind do |input_data|
                Queries::GroupUsersQuery.call(storage:, auth_strategy:, input_data:).value!
              end
            end

...
RSpec.describe Wikis::Adapters::Providers::XWiki::Queries::MentioningPages, :disable_ssrf_filter, :webmock do
  include XWikiStubs

  it "is registered" do
    expect(Wikis::Adapters::Registry.resolve("xwiki.queries.mentioning_pages")).to eq(described_class)
  end

  describe "input validation" do
    it "rejects a linkable that is not a work package" do
      expect(Wikis::Adapters::Input::MentioningPages.build(linkable: create(:project))).to be_failure
    end
  end
  ...
  describe "#call" do
    ...
    let(:input_data) { Wikis::Adapters::Input::MentioningPages.build(linkable:).value! }
    ...
    context "when the search request returns unauthorized" do
      before do
        stub_request(:get, mentions_endpoint(linkable, provider: wiki_provider)).to_return(status: 401, body: "")
      end
    ...
    context "when the search request times out" do
      before { stub_request(:get, mentions_endpoint(linkable, provider: wiki_provider)).to_timeout }

      it { is_expected.to be_failure.and have_attributes(failure: have_attributes(code: :connection_error)) }
    end

    context "with no mentioning pages in XWiki" do

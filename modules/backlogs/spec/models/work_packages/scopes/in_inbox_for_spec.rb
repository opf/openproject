...
RSpec.describe WorkPackages::Scopes::InInboxFor do
  ...
  shared_let(:open_bucket_wp) do
    create(:work_package, subject: "Bucket", project:, status: open_status, sprint: nil, backlog_bucket:)
  end
  ...
  subject(:backlog) { WorkPackage.in_inbox_for(project:) }

  describe ".in_inbox_for" do
    before do
    ...
    it "returns open work packages of the backlog inbox that are not excluded by type or status" do
      # Excludes:
      # - closed
      # - excluded type
      # - excluded status

...
RSpec.describe Backlogs::SprintReports::Widgets::RemovedTable, type: :component do
  ...
  context "with all permissions and entitlements", with_ee: %i[baseline_comparison sprint_report_pro_widgets] do
    context "when the sprint has no dates set" do
      ...
      include_examples "renders a blankslate" do
        let(:description) { "A sprint start and finish dates are necessary to show removed work packages." }
      end
    end
    ...
    context "when the sprint has not started" do
      ...
      include_examples "renders a blankslate" do
        let(:description) { "Removed work packages will appear here after the sprint starts." }
      end
    end
    ...
    context "when nothing was removed" do
      ...
      include_examples "renders a blankslate" do
        let(:description) { "Work packages that were removed after the sprint start date will appear here." }
      end
    end
    ...
    context "when the sprint is running" do
      ...
      it "renders a non baseline table" do
        expect(query_props["timestamps"]).to eq("#{started_at.iso8601},PT0S")
      end
    ...
    context "when the sprint is complete" do
      ...
      it "renders a baseline table using completion timestamp" do
        expect(query_props["timestamps"]).to eq("#{started_at.iso8601},#{completed_at.iso8601}")
      end

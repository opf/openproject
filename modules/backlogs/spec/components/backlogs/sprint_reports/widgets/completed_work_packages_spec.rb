...
RSpec.describe Backlogs::SprintReports::Widgets::CompletedTable, type: :component do
  include_examples "sprint report work package table widget" do
    ...
    let(:breakdown_overrides) do
      {
        done_status_ids: [1, 2],
        completed: SprintWorkPackageBreakdown::Block.new(work_package_count: completed_count, story_points: 8)
      }
    end
  end
  ...
  context "with all permissions and entitlements", with_ee: %i[baseline_comparison sprint_report_pro_widgets] do
    context "when the sprint has no dates set" do
      ...
      include_examples "renders a blankslate" do
        let(:description) { "A sprint start and finish dates are necessary to show completed work packages." }
      end
    end
    ...
    context "when the sprint has not started" do
      ...
      include_examples "renders a blankslate" do
        let(:description) { "Completed work packages will appear here after the sprint starts." }
      end
    end

    context "when nothing is completed" do
      let(:completed_count) { 0 }

      include_examples "renders a blankslate" do
        let(:description) { "Work packages that were completed in the sprint will appear here." }
      end
    end

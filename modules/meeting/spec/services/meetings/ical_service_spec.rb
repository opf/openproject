...
RSpec.describe Meetings::ICalService, type: :model do # rubocop:disable RSpec/SpecFilePathFormat
  ...
  let(:standard) { Icalendar::Calendar.parse(result).first.timezones.first.standards.first }
  let(:daylight) { Icalendar::Calendar.parse(result).first.timezones.first.daylights.first }
  ...
  describe "#call" do
    ...
    it "renders the ICS file", :aggregate_failures do
      ...
      expect(standard.dtstart.to_time.utc).to eq (Time.parse("01-11-2020 02:00:00").utc)
      expect(daylight.dtstart.to_time.utc).to eq (Time.parse("14-03-2021 04:00:00").utc)
    end
  end
end

...
RSpec.describe ResourcePlannerViews::DeleteContract do
  ...
  let(:view) { build_stubbed(:resource_work_package_list, parent: resource_planner, project:, principal: owner) }
  ...
  context "when user owns the parent planner with view_resource_planners permission" do
    let(:current_user) do
  ...
  context "when user can view planners but does not own this private one" do
    let(:current_user) do
      create(:user, member_with_permissions: { project => %i[view_resource_planners] })
    end
  ...
  context "when user has no resource planner permissions" do

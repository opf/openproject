...
RSpec.describe "Staffing requests",
  ...
  describe "GET index" do
    it "lists the unassigned generic allocations sorted by start date" do
      get resource_management_staffing_path(project_id: project)
  ...
  describe "the row context menu" do
    it "offers Assign inside a frame that reloads when an allocation changes" do
      ...
      expect(response.body).not_to include(edit_resource_allocation_path(earlier_allocation, project_id: project))
    ...
    context "with the allocate permission" do
      ...
      it "also offers Edit and Delete" do
        ...
        expect(response.body).to include(edit_resource_allocation_path(earlier_allocation, project_id: project))
  ...
  describe "the sidebar menu" do
    it "shows the Staffing entry for a user with the permission" do
      get menu_resource_planners_path(origin_controller: "resource_management/staffing", project_id: project)

      expect(response).to have_http_status(:ok)
      expect(response.body).to include(I18n.t("resource_management.staffing.menu_item"))
      expect(response.body).to include(resource_management_staffing_path(project_id: project))
    end
    ...
    context "without the assign permission" do
      ...
      it "hides the Staffing entry" do
        login_as other
        get menu_resource_planners_path(project_id: project)
  ...
  describe "GET assign (dialog)" do
    it "opens the dialog listing matching project members" do
      get resource_management_staffing_assign_path(earlier_allocation, project_id: project), as: :turbo_stream
  ...
  describe "PUT assign" do
    def assign(allocation, params = {})
      put resource_management_staffing_assign_path(allocation, project_id: project),

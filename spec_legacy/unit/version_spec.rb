...
describe Version, type: :model do
  ...
  it 'should create' do
    (d = Version.new.tap do |v|
      v.attributes = { project: Project.find(1),
                       name: '1.1',
  ...
  it 'should invalid effective date validation' do
    ...
    assert !d.save
    assert_includes(d.errors[:effective_date],
                    I18n.translate('activerecord.errors.messages.not_a_date'))
  end
  ...
  context '#start_date' do
    context 'with no value saved' do
      it 'should be the date of the earlist issue' do
        ...
        assert_equal '2010-03-01', d.start_date.to_s
      end
    end
    ...
    context 'with a value saved' do
      it 'should be the value' do
        ...
        add_work_package(d,
                         estimated_hours: 10,
                         start_date: '2010-03-01')
        assert_equal '2010-01-05', d.start_date.to_s
      end
    end
  end
  ...
  it 'should progress should be 0 with no assigned issues' do
    ...
    assert_equal 0, d.completed_percent
    assert_equal 0, d.closed_percent
  end
  ...
  it 'should progress should be 0 with unbegun assigned issues' do
    ...
    add_work_package(d)
    add_work_package(d, done_ratio: 0)
    assert_progress_equal 0, d.completed_percent
    assert_progress_equal 0, d.closed_percent
  end
  ...
  it 'should progress should be 100 with closed assigned issues' do
    ...
    add_work_package(d, status: status)
    add_work_package(d, status: status, done_ratio: 20)
    add_work_package(d, status: status, done_ratio: 70, estimated_hours: 25)
    add_work_package(d, status: status, estimated_hours: 15)
    assert_progress_equal 100.0, d.completed_percent
    assert_progress_equal 100.0, d.closed_percent
  end
  ...
  it 'should progress should consider done ratio of open assigned issues' do
    ...
    add_work_package(d)
    add_work_package(d, done_ratio: 20)
    add_work_package(d, done_ratio: 70)
    assert_progress_equal (0.0 + 20.0 + 70.0) / 3, d.completed_percent
    assert_progress_equal 0, d.closed_percent
  end
  ...
  it 'should progress should consider closed issues as completed' do
    ...
    add_work_package(d)
    add_work_package(d, done_ratio: 20)
    add_work_package(d, status: Status.where(is_closed: true).first)
    assert_progress_equal (0.0 + 20.0 + 100.0) / 3, d.completed_percent
    assert_progress_equal (100.0 / 3), d.closed_percent
  end
  ...
  it 'should progress should consider estimated hours to weigth issues' do
    ...
    add_work_package(d, estimated_hours: 10)
    add_work_package(d, estimated_hours: 20, done_ratio: 30)
    add_work_package(v, estimated_hours: 40, done_ratio: 10)
    add_work_package(d, estimated_hours: 25, status: Status.where(is_closed: true).first)
    assert_progress_equal((10.0 * 0 + 20.0 * 0.3 + 40 * 0.1 + 25.0 * 1) / 95.0 * 100, d.completed_percent)
    assert_progress_equal 25.0 / 95.0 * 100, d.closed_percent
  end
  ...
  it 'should progress should consider average estimated hours to weigth unestimated issues' do
    ...
    add_work_package(d, done_ratio: 20)
    add_work_package(d, status: Status.where(is_closed: true).first)
    add_work_package(d, estimated_hours: 10, done_ratio: 30)
    add_work_package(d, estimated_hours: 40, done_ratio: 10)
    assert_progress_equal((25.0 * 0.2 + 25.0 * 1 + 10.0 * 0.3 + 40.0 * 0.1) / 100.0 * 100, d.completed_percent)
    assert_progress_equal 25.0 / 100.0 * 100, d.closed_percent
  end
    ...
    it 'should be false if all of the issues are ahead of schedule' do
      ...
      @version.fixed_issues = [
        FactoryGirl.create(:work_package,
                           ...
                           done_ratio: 60), # 14 day span, 60% done, 50% time left
        FactoryGirl.create(:work_package,
                           project: @project,
                           start_date: 7.days.ago,
                           done_ratio: 60) # 14 day span, 60% done, 50% time left
    ...
    it 'should be true if any of the issues are behind schedule' do
      ...
      @version.fixed_issues = [
        ...
        FactoryGirl.create(:work_package,
                           project: @project,
                           start_date: 7.days.ago,
                           done_ratio: 20) # 14 day span, 20% done, 50% time left
      ]
    ...
    it 'should be false if all of the issues are complete' do
      ...
      @version.fixed_issues = [
        FactoryGirl.create(:work_package,
                           ...
                           done_ratio: 100,
                           status: Status.find(5)), # 7 day span
        FactoryGirl.create(:work_package,
                           project: @project,
                           start_date: 14.days.ago,
                           done_ratio: 100,
  ...
  def add_work_package(version, attributes = {})
    (d = WorkPackage.new.tap do |v|
      v.attributes = { project: version.project,
                       fixed_version: version,
                       subject: 'Test',
                       author: User.first,
                       type: version.project.types.first }
        .merge(attributes)
    end).save!
    d
  end
  ...
  def assert_progress_equal(expected_float, actual_float, _message = '')
    assert_in_delta(expected_float, actual_float, 0.000001, _message = '')
  end
end

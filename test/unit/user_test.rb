...
describe User do
  ...
  it 'should mail_should_be_stripped' do
    u = User.new
    u.mail = " foo@bar.com  "
  ...
  context "User#before_create" do
    it "set the mail_notification to the default Setting" do
  ...
  it 'should mail_uniqueness_should_not_be_case_sensitive' do
    u = User.new(:firstname => "new", :lastname => "user", :mail => "newuser@somenet.foo")
    u.login = 'newuser1'
    u.password, u.password_confirmation = "adminADMIN!", "adminADMIN!"
    assert u.save
  ...
  it 'should validate_login_presence' do
    @admin.login = ""
    assert !@admin.save
    assert_equal 1, @admin.errors.count
  end

  it 'should validate_mail_notification_inclusion' do
    u = User.new
  ...
  context "User#try_to_login" do
    it "fall-back to case-insensitive if user login is not found as-typed." do
      user = User.try_to_login("AdMin", "adminADMIN!")
      assert_kind_of User, user
      assert_equal "admin", user.login
    end

    it "select the exact matching user first" do
    ...
    context "#try_to_login using LDAP" do
      ...
      context "on the fly registration" do
        ...
        context "with a successful authentication" do
          it "create a new user account if it doesn't exist" do
  ...
  context "User#api_key" do
    it "generate a new one if the user doesn't have one" do
    ...
    it "return the existing api token value" do
  ...
  context "User#find_by_api_key" do
    it "return nil if no matching key is found" do
      assert_nil User.find_by_api_key('zzzzzzzzz')
    end

    it "return nil if the key is found for an inactive user" do
    ...
    it "return the user if the key is found for an active user" do
  ...
  it 'should roles_for_project' do
    # user with a role
  ...
  it 'should projects_by_role_for_user_with_role' do
    user = User.find(2)
  ...
  it 'should projects_by_role_for_user_with_no_role' do
    user = User.generate!
    assert_equal({}, user.projects_by_role)
  end

  it 'should projects_by_role_for_anonymous' do
    assert_equal({}, User.anonymous.projects_by_role)
  end

  it 'should valid_notification_options' do
    # without memberships
  ...
  it 'should valid_notification_options_class_method' do
  ...
  it 'should mail_notification_all' do
  ...
  it 'should mail_notification_selected' do
  ...
  it 'should mail_notification_only_my_events' do
  ...
  it 'should comments_sorting_preference' do
  ...
  it 'should find_by_mail_should_be_case_insensitive' do
  ...
  context "#change_password_allowed?" do
    it "be allowed if no auth source is set" do
      user = User.generate_with_protected!
      assert user.change_password_allowed?
    end

    it "delegate to the auth source" do
      user = User.generate_with_protected!
  ...
  context "#allowed_to?" do
    context "with a unique project" do
      it "return false if project is archived" do
      ...
      it "return false if related module is disabled" do
        project = Project.find(1)
      ...
      it "authorize nearly everything for admin users" do
        project = Project.find(1)
      ...
      it "authorize normal users depending on their roles" do
        project = Project.find(1)
      ...
      it "only managers are allowed to export tickets" do
        project = Project.find(1)
        project.enabled_module_names = ["work_package_tracking"]
    ...
    context "with multiple projects" do
      ...
      it "return true only if user has permission on all these projects" do
      ...
      it "behave correctly with arrays of 1 project" do
        assert ! User.anonymous.allowed_to?(:delete_work_packages, [Project.first])
      end
    end

    context "with options[:global]" do
      it "authorize if user has at least one role that has this permission" do
  ...
  context "User#notify_about?" do
    context "Issues" do
      ...
      it "be true for a user with :all" do
        @author.update_attribute(:mail_notification, 'all')
        assert @author.notify_about?(@issue)
      end

      it "be false for a user with :none" do
        @author.update_attribute(:mail_notification, 'none')
        assert ! @author.notify_about?(@issue)
      end

      it "be false for a user with :only_my_events and isn't an author, creator, or assignee" do
      ...
      it "be true for a user with :only_my_events and is the author" do
        @author.update_attribute(:mail_notification, 'only_my_events')
        assert @author.notify_about?(@issue)
      end

      it "be true for a user with :only_my_events and is the assignee" do
        @assignee.update_attribute(:mail_notification, 'only_my_events')
        assert @assignee.notify_about?(@issue)
      end

      it "be true for a user with :only_assigned and is the assignee" do
        @assignee.update_attribute(:mail_notification, 'only_assigned')
        assert @assignee.notify_about?(@issue)
      end

      it "be false for a user with :only_assigned and is not the assignee" do
        @author.update_attribute(:mail_notification, 'only_assigned')
        assert ! @author.notify_about?(@issue)
      end

      it "be true for a user with :only_owner and is the author" do
        @author.update_attribute(:mail_notification, 'only_owner')
        assert @author.notify_about?(@issue)
      end

      it "be false for a user with :only_owner and is not the author" do
        @assignee.update_attribute(:mail_notification, 'only_owner')
        assert ! @assignee.notify_about?(@issue)
      end

      it "be true for a user with :selected and is the author" do
        @author.update_attribute(:mail_notification, 'selected')
        assert @author.notify_about?(@issue)
      end

      it "be true for a user with :selected and is the assignee" do
        @assignee.update_attribute(:mail_notification, 'selected')
        assert @assignee.notify_about?(@issue)
      end

      it "be false for a user with :selected and is not the author or assignee" do
    ...
    context "other events" do
      it 'be added and tested'
    end
  end

  if Object.const_defined?(:OpenID)

  it 'should setting_identity_url' do
    normalized_open_id_url = 'http://example.com/'
    u = User.new( :identity_url => 'http://example.com/' )
    assert_equal normalized_open_id_url, u.identity_url
  end

  it 'should setting_identity_url_without_trailing_slash' do
    normalized_open_id_url = 'http://example.com/'
    u = User.new( :identity_url => 'http://example.com' )
    assert_equal normalized_open_id_url, u.identity_url
  end

  it 'should setting_identity_url_without_protocol' do
    normalized_open_id_url = 'http://example.com/'
    u = User.new( :identity_url => 'example.com' )
    assert_equal normalized_open_id_url, u.identity_url
  end

  it 'should setting_blank_identity_url' do
    u = User.new( :identity_url => 'example.com' )
    u.identity_url = ''
    assert u.identity_url.blank?
  end

  it 'should setting_invalid_identity_url' do

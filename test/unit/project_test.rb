...
class ProjectTest < ActiveSupport::TestCase
  ...
  context "associations" do
    ...
    should have_many(:work_unit_changes).through(:work_units)
    ...
    should have_and_belong_to_many :work_unit_custom_fields
  end
  ...
  def test_destroying_root_projects_should_clear_data
    ...
    assert_equal 0, WorkUnit.count
    assert_equal 0, WorkUnitJournal.count, "Journals were not deleted: #{Journal.all.inspect}"
  ...
  context "Project#copy" do
    ...
    should "copy work units" do
      @source_project.work_units << Issue.generate!(:status => IssueStatus.find_by_name('Closed'),
                                                    :subject => "copy issue status",
                                                    :tracker_id => 1,
                                                    :assigned_to_id => 2,
                                                    :project_id => @source_project.id)
      assert @project.valid?
      assert @project.work_units.empty?
      assert @project.copy(@source_project)

      assert_equal @source_project.work_units.size, @project.work_units.size
      ...
      copied_issue = @project.work_units.first(:conditions => {:subject => "copy issue status"})
      assert copied_issue
    ...
    should "change the new issues to use the copied version" do
      ...
      assert @project.copy(@source_project)
      @project.reload
      copied_issue = @project.work_units.first(:conditions => {:subject => "change the new issues to use the copied version"})
    ...
    should "copy issue relations" do
      ...
      assert @project.copy(@source_project)
      assert_equal @source_project.work_units.count, @project.work_units.count
      copied_issue = @project.work_units.find_by_subject("Issue on project 2") # Was #4
      copied_second_issue = @project.work_units.find_by_subject("copy issue relation")
    ...
    should "limit copy with :only option" do
      ...
      assert @source_project.work_units.any?

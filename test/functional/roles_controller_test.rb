...
class RolesControllerTest < ActionController::TestCase
  ...
  def test_post_new_with_validaton_failure
    post :create, :role => { :name => '',
                             :permissions => ['add_issues', 'edit_work_units', 'log_time', ''],
                             :assignable => '0' }
  ...
  def test_post_new_without_workflow_copy
    ...
    assert_not_nil role
    assert_equal [:add_issues, :edit_work_units, :log_time], role.permissions
  ...
  def test_put_bulk_update
    put :bulk_update, :permissions => { '0' => '', '1' => ['edit_work_units'], '3' => ['add_issues', 'delete_issues']}
    assert_redirected_to roles_path

    assert_equal [:edit_work_units], Role.find(1).permissions

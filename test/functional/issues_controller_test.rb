...
class IssuesControllerTest < ActionController::TestCase
  ...
  context "without workflow privilege" do
    ...
    context "#update" do
      should "ignore status change" do
        assert_difference 'WorkUnitJournal.count' do
  ...
  def test_put_update_with_status_and_assignee_change
    ...
    issue.reload
    assert_equal 2, issue.status_id
    j = WorkUnitJournal.find(:first, :order => 'id DESC')
  ...
  def test_put_update_with_no_change
    ...
    assert_no_difference('WorkUnitJournal.count') do
      put :update,
           :id => 1,
           :notes => ''
    end
  ...
  def test_bulk_update_on_different_projects_without_rights
    ...
    put :bulk_update, :ids => [1, 6], :notes => 'Bulk should fail',
                                     :issue => {:priority_id => 7,
                                                ...
                                                :custom_field_values => {'2' => ''}}
    assert_response 403
    assert_not_equal "Bulk should fail", WorkUnitJournal.last.notes
  end
  ...
  def test_destroy_issue_with_no_time_entries
    assert_nil TimeEntry.find_by_work_unit_id(2)
    @request.session[:user_id] = 2
    post :destroy, :id => 2
  ...
  def test_destroy_issues_and_assign_time_entries_to_project
    ...
    assert_redirected_to :action => 'index', :project_id => 'ecookbook'
    assert !(Issue.find_by_id(1) || Issue.find_by_id(3))
    assert_nil TimeEntry.find(1).work_unit_id
    assert_nil TimeEntry.find(2).work_unit_id
  end
  ...
  def test_destroy_issues_and_reassign_time_entries_to_another_issue
    ...
    assert_redirected_to :action => 'index', :project_id => 'ecookbook'
    assert !(Issue.find_by_id(1) || Issue.find_by_id(3))
    assert_equal 2, TimeEntry.find(1).work_unit_id
    assert_equal 2, TimeEntry.find(2).work_unit_id
  end

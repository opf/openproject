...
class RedCloth3 < String
  ...
  # set of rules with the single :textile rule,
  # which supplies the rule set in
  ...
  # block_textile_prefix:: Textile blocks with prefixes
  # (i.e. bq., h2., etc.)
  ...
  #
  # refs_markdown::
  # Markdown references (for example: [hobix]: http://hobix.com/)
  ...
  def to_html(*rules)
    ...
    textile_rules = %i(block_textile_table
                       ...
                       inline_textile_span
                       glyphs_textile)
    markdown_rules = %i(refs_markdown
  ...
  def block_markdown_rule(text)
    text.gsub!(MARKDOWN_RULE_RE) do |_blk|
      "<hr />"
    end
  end
  ...
  def textile_popup_help(name, windowW, windowH)
    '<a target="_blank href="http://hobix.com/textile/#' + helpvar +
      '" onclick="window.open(this.href, \'popupwindow\', \'width=' +
      windowW + ',height=' + windowH +
      ',scrollbars,resizable\'); return false;">' + name + '</a><br />'
  end

  # HTML cleansing stuff
  BASIC_TAGS = { 'a' => ['href', 'title'],
                 'img' => ['src', 'alt', 'title'],
                 'br' => [],
                 'i' => nil,
                 'u' => nil,

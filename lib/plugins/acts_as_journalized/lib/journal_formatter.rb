...
module JournalFormatter
  ...
  def formatter_instance(formatter_key)
    ...
    formatter = JournalFormatter.registered_fields[journaled_type.to_sym].keys.detect{ |k| formatter_key.match(k) }

    return nil if formatter.nil?

    @formatter_instances ||= Hash.new do |hash, key|
      f = JournalFormatter.formatters[JournalFormatter.registered_fields[journaled_type.to_sym][key]]
      hash[key] = f.new(self)
    end

...
if [ "$1" == "run-units" ]; then
	...
	execute "time bundle exec turbo_tests -n $JOBS --verbose --runtime-log spec/support/turbo_runtime_units.log spec"
...
if [ "$1" == "run-features" ]; then
	...
	execute "time bundle exec turbo_tests -n $JOBS --verbose --runtime-log spec/support/turbo_runtime_features.log spec/features"

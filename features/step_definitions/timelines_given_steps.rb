...
Given /^there are the following planning element types:$/ do |table|
  ...
  table.hashes.each do |type_attributes|
    FactoryGirl.create(:timelines_planning_element_type, type_attributes)
  end
end
...
Given /^there are the following planning element statuses:$/ do |table|
  ...
  table.hashes.each do |type_attributes|
    FactoryGirl.create(:timelines_planning_element_status, type_attributes)
  end
end
...
Given /^there are the following reported project statuses:$/ do |table|
  ...
  table.hashes.each do |type_attributes|
    FactoryGirl.create(:timelines_reported_project_status, type_attributes)
  end
end
...
Given /^there are the following project types:$/ do |table|
  ...
  table.hashes.each do |type_attributes|
    FactoryGirl.create(:timelines_project_type, type_attributes)
  end
end
...
Given /^the following planning element types are default for projects of type "([^"]*)"$/ do |project_type_name, pe_type_names|
  ...
  pe_type_names.each do |pe_type_name|
    FactoryGirl.create(:timelines_default_planning_element_type,
                   ...
                   :planning_element_type_id => Timelines::PlanningElementType.find_by_name!(pe_type_name).id)
  end
end
...
Given /^there are the following alternate dates for "([^"]*)":$/ do |scenario_name, table|
  ...
  table.hashes.each do |row|
    planning_element = Timelines::PlanningElement.find_by_subject!(row["planning_element_subject"])
...
Given /^there is a project named "([^"]*)" of type "([^"]*)"$/ do |name, project_type_name|
  FactoryGirl.create(:project,
                 :name                      => name,
                 :timelines_project_type_id => Timelines::ProjectType.find_by_name!(project_type_name).id)
end
...
Given /^the project(?: named "([^"]*)")? has no project type$/ do |name|
  ...
  project.update_attribute(:timelines_project_type_id, nil)
end
...
Given /^there are the following project associations:$/ do |table|
  ...
  table.hashes.each do |type_attributes|
    FactoryGirl.create(:timelines_project_association, type_attributes)
  end
end
...
Given /^there are the following reportings:$/ do |table|
  ...
  table.hashes.each do |attrs|
    ...
    FactoryGirl.create(:timelines_reporting, attrs)
  end
end

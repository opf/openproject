...
module WatchersHelper
  ...
  def watchers_list(object)
    ...
    lis = object.watcher_users.sort.collect do |watch|
      watcher = object.watchers(true).find{|u| u.user_id == watch.id }
      content_tag :li do
        avatar(watch, :size => "16") +
          link_to_user(watch, :class => 'user') +
          if remove_allowed
            ' '.html_safe + link_to(icon_wrapper('icon-context icon-close delete-ctrl',
                                                 l(:button_delete_watcher, name: watch.name)),
                             watcher_path(watcher),
                             :method => :delete,
                             :remote => true,
                             :style => "vertical-align: middle",
                             :title => l(:button_delete_watcher, name: watch.name),

...
class TimeEntries::ReportsController < ApplicationController
  ...
  def load_available_criterias
    ...
    custom_fields.select { |cf| %w(list bool).include? cf.field_format }.each do |cf|
      @available_criterias["custom_field_#{cf.id}"] = { sql: "(SELECT c.value FROM #{CustomValue.table_name} c WHERE c.custom_field_id = #{cf.id} AND c.customized_type = 'WorkPackage' AND c.customized_id = #{WorkPackage.table_name}.id)",
    ...
    TimeEntryCustomField.find(:all).select { |cf| %w(list bool).include? cf.field_format }.each do |cf|
      @available_criterias["custom_field_#{cf.id}"] = { sql: "(SELECT c.value FROM #{CustomValue.table_name} c WHERE c.custom_field_id = #{cf.id} AND c.customized_type = 'TimeEntry' AND c.customized_id = #{TimeEntry.table_name}.id)",
                                              format: cf.field_format,
                                              label: cf.name }
    end
    ...
    TimeEntryActivityCustomField.find(:all).select { |cf| %w(list bool).include? cf.field_format }.each do |cf|
      @available_criterias["custom_field_#{cf.id}"] = { sql: "(SELECT c.value FROM #{CustomValue.table_name} c WHERE c.custom_field_id = #{cf.id} AND c.customized_type = 'Enumeration' AND c.customized_id = #{TimeEntry.table_name}.activity_id)",
                                              format: cf.field_format,
                                              label: cf.name }
    end

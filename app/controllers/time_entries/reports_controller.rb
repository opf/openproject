...
class TimeEntries::ReportsController < ApplicationController
  ...
  def show
    ...
    unless @criterias.empty?
      ...
      if @project.nil?
        ...
        sql_condition = "#{WorkUnit.table_name}.root_id = #{@issue.root_id} AND #{WorkUnit.table_name}.lft >= #{@issue.lft} AND #{WorkUnit.table_name}.rgt <= #{@issue.rgt}"
      end
  ...
  def load_available_criterias
    @available_criterias = { 'project' => {:sql => "#{TimeEntry.table_name}.project_id",
                             ...
                             'version' => {:sql => "#{WorkUnit.table_name}.fixed_version_id",
                                          :klass => Version,
                                          :label => Version.model_name.human},
                             'category' => {:sql => "#{WorkUnit.table_name}.category_id",
                                            :klass => IssueCategory,
                                            :label => IssueCategory.model_name.human},
                             'member' => {:sql => "#{TimeEntry.table_name}.user_id",
                                         :klass => User,
                                         :label => Member.model_name.human},
                             'tracker' => {:sql => "#{WorkUnit.table_name}.tracker_id",
                                          :klass => Tracker,

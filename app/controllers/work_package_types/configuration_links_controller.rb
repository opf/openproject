...
module WorkPackageTypes
  # "Mirror another type" / "Change source type" on a type's configuration
  # tabs: the source-picker dialog, the danger confirmation (whose wording
  # depends on whether the configuration is currently manual or mirrored), and the
  ...
  class ConfigurationLinksController < BaseTabController
    ...
    def confirm
      if source.nil?
        render_error_flash_message_via_turbo_stream(message: t("types.edit.reuse_mode.mirrored.invalid_source"))
      else
    ...
    def respond_to_switch(result)
      if result.success?
        render_success_flash_message_via_turbo_stream(message: t("types.edit.reuse_mode.mirrored.success"))

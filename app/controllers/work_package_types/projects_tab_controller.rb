...
module WorkPackageTypes
  class ProjectsTabController < BaseTabController
    ...
    def sync_projects(desired)
      ...
      all_or_nothing do |aggregated|
        apply_to(::Project.where(id: desired - applying), into: aggregated)
        remove_from(::Project.where(id: applying - desired), into: aggregated)
      end
    end

    # The per-project services each succeed or fail on their own, so without this the projects that
    # could be written stay written while the error only names the ones that could not — leaving the
    # tab reporting a failed save it half applied.
    def all_or_nothing

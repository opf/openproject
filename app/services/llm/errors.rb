...
module Llm
  ...
  module Errors
    ...
    # The original message is the only place the upstream detail exists, and an
    # administrator debugging a gateway needs it -- it just must not travel back
    # up to a view or a contract error.
    def log(error)
      Rails.logger.info { "LLM request failed: #{error.class}: #{error.message}" }
    end
  end
end

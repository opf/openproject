...
class Authorization::WorkPackageQuery < Authorization::AbstractQuery
  self.model = WorkPackage

  def self.entities_members_join(user)
    work_packages_table[:id]
      .eq(members_table[:entity_id])
      .and(members_table[:user_id].eq(user.id))
      .and(members_table[:entity_type].eq(model.name.to_s))
  end

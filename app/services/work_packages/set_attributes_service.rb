...
class WorkPackages::SetAttributesService < BaseServices::SetAttributes
  ...
  def set_attributes(attributes)
    ...
    set_categories_attributes(attributes)
  ...
  # Routed through the replacements accessor rather than the association writer
  # generated by has_many, which would write to the database before the contract
  # ever gets to validate. Removing the key also keeps set_static_attributes from
  # picking up `category_ids=`.
  def set_categories_attributes(attributes)
    category_ids = attributes.delete(:category_ids)

    model.category_ids_replacements = Array(category_ids).map(&:to_i) if category_ids
  end
  ...
  def reassign_categories
    ...
    # reassign every category to the one of the same name in the new project,
    # dropping the ones that have no counterpart there
    current_categories = work_package.effective_categories
    return if current_categories.empty?

    # Project#categories is name-ordered, so the first match is the one that gets
    # mirrored into the deprecated column on save.
    reassigned = work_package.project.categories.where(name: current_categories.map(&:name)).to_a

    work_package.category_ids_replacements = reassigned.map(&:id)
    # Without this the deprecated column would still point at a category of the
    # old project, which the contract rejects. The save re-derives it anyway.
    work_package.category = reassigned.first
  end

...
class TimeEntry < ActiveRecord::Base
  ...
  acts_as_journalized :event_title => Proc.new {|o| "#{l_hours(o.hours)} (#{(o.work_unit || o.project).event_title})"},
                :event_url => Proc.new {|o| {:controller => '/timelog', :action => 'index', :project_id => o.project, :work_unit_id => o.work_unit}},
                :event_author => :user,
  ...
  validate :validate_consistency_of_work_unit_id
  ...
  safe_attributes 'hours', 'comments', 'work_unit_id', 'activity_id', 'spent_on', 'custom_field_values'
  ...
  def set_default_project
    self.project ||= work_unit.project if work_unit
  end
...
private
  ...
  def validate_consistency_of_work_unit_id
    errors.add :work_unit_id, :invalid if (work_unit_id && !work_unit) || (work_unit && project!=work_unit.project)
  end
end

...
class Automation < ApplicationRecord
  ...
  scope :with_button_trigger, -> {
    joins(:triggers).where(automation_triggers: { type: "Automations::Triggers::Button" }).distinct
  }
  ...
  def must_not_have_more_than_one_button_trigger
    button_count = triggers
                     .reject(&:marked_for_destruction?)
                     .count { |trigger| trigger.is_a?(Automations::Triggers::Button) }
    errors.add(:triggers, :invalid) if button_count > 1
  end
end

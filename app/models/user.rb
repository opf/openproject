...
##
# Load dependency if not defined, otherwise run given block
# 
# This function makes ActiveSupport::Dependencies think, it hasn't tried to load 
# klass before, by removing it from the list of loaded modules.
# It then first loads the dependency by accessing the dependency constant, which triggers
# the auto-loading. Once the dependency is loaded, the dependency probably already tried
# to access klass, which triggered auto-loading again. To make sure it's loaded, this
# function also tries to access klass. The second time auto-loading of klass is triggered,
# dependecy is defined and the given block is executed.
def enforce_load_order dependency, klass
  unless Object.const_defined? dependency
    user_file = ActiveSupport::Dependencies.search_for_file klass.to_s.underscore
    ActiveSupport::Dependencies.loaded.delete(user_file.sub(/\.rb\z/, ''))

    # Force ActiveSupport::Dependencies to load klass.
    # When reloading is disabled, it otherwise uses require, which doesn't try to load
    # already loaded files again.
    old_mechanism = ActiveSupport::Dependencies.mechanism
    ActiveSupport::Dependencies.mechanism = :load

    # Load dependency
    Object.const_get dependency

    # Load klass itself, in case dependency didn't already load it
    Object.const_get klass

    # Reset load mechanism
...
enforce_load_order :Principal, :User do

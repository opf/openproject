...
class Query < ActiveRecord::Base
  ...
  @@available_columns = [
    ...
    QueryColumn.new(:parent, :sortable => ["#{WorkUnit.table_name}.root_id", "#{WorkUnit.table_name}.lft ASC"], :default_order => 'desc', :caption => :parent_issue),
    ...
    QueryColumn.new(:start_date, :sortable => ["CASE WHEN #{WorkUnit.table_name}.start_date IS NULL THEN 1 ELSE 0 END", "#{WorkUnit.table_name}.start_date"]),
    QueryColumn.new(:due_date, :sortable => ["CASE WHEN #{WorkUnit.table_name}.due_date IS NULL THEN 1 ELSE 0 END", "#{WorkUnit.table_name}.due_date"]),
  ...
  def available_filters
    ...
    if project
      ...
      add_custom_fields_filters(project.all_work_unit_custom_fields)
    else
      ...
      add_custom_fields_filters(WorkUnitCustomField.find(:all, :conditions => {:is_filter => true, :is_for_all => true}))
    end
    @available_filters
  end
  ...
  def available_columns
    ...
    @available_columns += (project ?
                            project.all_work_unit_custom_fields :
                            WorkUnitCustomField.find(:all)
  ...
  def sortable_columns
    {'id' => "#{WorkUnit.table_name}.id"}.merge(available_columns.inject({}) {|h, column|
  ...
  def project_statement
    ...
    project_clauses <<  WorkUnit.visible_condition(User.current)
  ...
  def statement
    ...
    filters.each_key do |field|
      ...
      if field =~ /^cf_(\d+)$/
        ...
        sql << "#{WorkUnit.table_name}.id IN (SELECT #{WorkUnit.table_name}.id FROM #{WorkUnit.table_name} LEFT OUTER JOIN #{db_table} ON #{db_table}.customized_type='WorkUnit' AND #{db_table}.customized_id=#{WorkUnit.table_name}.id AND #{db_table}.custom_field_id=#{$1} WHERE "
        ...
        if User.current.admin?
          ...
          sql << "#{WorkUnit.table_name}.id #{operator == '=' ? 'IN' : 'NOT IN'} (SELECT #{db_table}.watchable_id FROM #{db_table} WHERE #{db_table}.watchable_type='WorkUnit' AND #{sql_for_field field, '=', v, db_table, db_field})"
        else
          sql_parts = []
          if User.current.logged? && user_id = v.delete(User.current.id.to_s)
            # a user can always see his own watched issues
            sql_parts << "#{WorkUnit.table_name}.id #{operator == '=' ? 'IN' : 'NOT IN'} (SELECT #{db_table}.watchable_id FROM #{db_table} WHERE #{db_table}.watchable_type='WorkUnit' AND #{sql_for_field field, '=', [user_id], db_table, db_field})"
          end
          ...
          sql_parts << "#{WorkUnit.table_name}.id #{operator == '=' ? 'IN' : 'NOT IN'} (SELECT #{db_table}.watchable_id FROM #{db_table} WHERE #{db_table}.watchable_type='WorkUnit' AND #{sql_for_field field, '=', v, db_table, db_field})"\
        ...
        sql << '(' + sql_for_field("assigned_to_id", operator, members_of_groups, WorkUnit.table_name, "assigned_to_id", false) + ')'
        ...
        sql << '(' + sql_for_field("assigned_to_id", operator, members_of_roles, WorkUnit.table_name, "assigned_to_id", false) + ')'
      else
        # regular field
        db_table = WorkUnit.table_name
        db_field = field
  ...
  def issue_count
    WorkUnit.count(:include => [:status, :project], :conditions => statement)
  ...
  def issue_count_by_group
    ...
    if grouped?
      begin
        ...
        r = WorkUnit.count(:group => group_by_statement, :include => [:status, :project], :conditions => statement)
  ...
  def issues(options={})
    ...
    WorkUnit.find :all, :include => ([:status, :project] + (options[:include] || [])).uniq,
  ...
  def work_unit_journals(options={})
    WorkUnitJournal.find :all, :joins => [:user, {:work_unit => [:project, :author, :tracker, :status]}],

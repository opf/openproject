...
class Message < ApplicationRecord
  ...
  def editable_by?(usr)
    usr && usr.logged? && (usr.allowed_in_project?(:edit_messages,
                                                   project) || (author == usr && usr.allowed_in_project?(:edit_own_messages,
                                                                                                         project)))
  end

  def destroyable_by?(usr)
    usr && usr.logged? && (usr.allowed_in_project?(:delete_messages,
                                                   project) || (author == usr && usr.allowed_in_project?(:delete_own_messages,
                                                                                                         project)))
  end

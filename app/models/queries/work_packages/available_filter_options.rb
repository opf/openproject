module Queries::WorkPackages::AvailableFilterOptions
  ...
  def add_user_options(user)
    ...
    assigned_to_values = [["<< #{l(:label_me)} >>", "me"]] + assigned_to_values if user.logged?
    ...
    author_values = []
    author_values << ["<< #{l(:label_me)} >>", "me"] if user.logged?
    ...
    responsible_values = [["<< #{l(:label_me)} >>", "me"]] + responsible_values if user.logged?
    ...
    if user.logged?
      ...
      user_values.each { |v| watcher_values << v } if user.allowed_to_globally?(:view_work_packages_watchers, {})

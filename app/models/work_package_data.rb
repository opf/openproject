...
class WorkPackageData < ActiveRecord::Base
  ...
  has_many :time_entries, :foreign_key => "work_package_id",
                          :dependent => :delete_all
  has_many :relations_from, :class_name => 'IssueRelation', :foreign_key => 'issue_from_id', :dependent => :delete_all
  has_many :relations_to, :class_name => 'IssueRelation', :foreign_key => 'issue_to_id', :dependent => :delete_all

  scope :recently_updated, :order => "#{WorkPackageData.table_name}.updated_at DESC"
  scope :visible, lambda {|*args| { :include => :project,
                                    :conditions => WorkPackageData.visible_condition(args.first ||
                                                                                 User.current) } }
  scope :without_deleted, :conditions => "#{WorkPackageData.quoted_table_name}.deleted_at IS NULL"
  scope :deleted, :conditions => "#{WorkPackageData.quoted_table_name}.deleted_at IS NOT NULL"

  acts_as_watchable

  acts_as_nested_set :scope => 'root_id', :dependent => :destroy
  acts_as_customizable :class_name => "WorkPackageCustomField"
  ...
  acts_as_journalized :class_name => "WorkPackageJournal",
                      :journal_class_association_name => :work_package,
                      :activity_type => 'work_packages',
                      :activity_permission => :view_work_packages,
                      :event_title => Proc.new {|o| "#{ o.to_s }"},
  ...
  WorkPackageCustomField.class_eval do
  ...
  def copy_from(arg, options = {})
    ...
    work_package = arg.is_a?(WorkPackageData) ? arg : WorkPackageData.visible.find(arg)
  ...
  def recalculate_attributes_for(work_package_id)
    if work_package_id.is_a? WorkPackageData
      p = work_package_id
    else
      p = WorkPackageData.find_by_id(work_package_id)
    end
    ...
    if p
      ...
      unless WorkPackageData.use_status_for_done_ratio? && p.status && p.status.default_done_ratio

...
module WorkPackageTypes
  class VariantComparison
    ...
    def same_as_type?(profile)
      !profile.base? && !base_profile.nil? && profile.digest == base_profile.digest
    end

    def duplicates_of(profile)
      return [] if profile.base? || same_as_type?(profile)

      duplicate_groups.fetch(profile.digest, []).reject { it.id == profile.id || it.base? }
    end
    ...
    def configuration_rows
      ASPECT_LABEL_KEYS.map do |aspect, label_key|
        Row.new(section: :configuration, key: aspect, aspect:, label_key:,
                reader: ->(_) { [] }, format: :mode, indented: false)
      end
    end

    def scope_rows
      %i[availability projects enabled_in_new_projects].map do |key|
        Row.new(section: :scope, key:, aspect: nil, label_key: "types.comparison.rows.scope.#{key}",
                reader: ->(_) { [] }, format: :plain, indented: false)
      end
    end
    ...
    def form_rows
      [same_as_base_row(:form, FORM),
       ...
       row(:form, :fields, FORM, :fields),
       row(:form, :required, FORM, :required, format: :list, indented: true)]
    end
    ...
    def workflow_rows
      [same_as_base_row(:workflows, WORKFLOWS),
       ...
       row(:workflows, :roles, WORKFLOWS, :roles, indented: true)]
    end
    ...
    def same_as_base_row(section, aspect)
      Row.new(section:, key: :same_as_base, aspect:,
              ...
              reader: ->(_) { [] }, format: :same_as_base, indented: false)
    end

    def row(section, key, aspect, reader, format: :count, indented: false)
      Row.new(section:, key:, aspect:, label_key: "types.comparison.rows.#{section}.#{key}",
              reader: ->(profile) { profile.public_send(reader) }, format:, indented:)
    end
    ...
    def workflow_of(variant)
      workflow = transitions_by_workflow.fetch(variant.workflow_id, EMPTY_WORKFLOW)
    ...
    # Variants sharing a workflow share its id, so the transitions are read once per workflow
    # rather than once per variant.
    def transitions_by_workflow
      @transitions_by_workflow ||=
        ::Workflows::StatusTransition
          .where(workflow_id: variants.map(&:workflow_id).uniq)
          .pluck(:workflow_id, :role_id, :old_status_id, :new_status_id)
          .group_by(&:first)
    ...
    def statuses
      @statuses ||= ::Status.where(id: transitions_by_workflow.values.flat_map { it[:status_ids] }.uniq).index_by(&:id)
    end

    def roles
      @roles ||= ::Role.where(id: transitions_by_workflow.values.flat_map { it[:role_ids] }.uniq).index_by(&:id)
    end

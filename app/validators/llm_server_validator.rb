...
class LlmServerValidator < ActiveModel::EachValidator
  ...
  # Only formats with live model discovery are probed. For the other formats the
  # list comes from the model registry, so there is nothing at this URL to probe.
  def queries_the_server?(contract)
    Llm::Adapters.live_discovery?(contract.model.api_format)
  end

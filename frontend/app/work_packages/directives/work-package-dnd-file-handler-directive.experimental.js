 ...
 **/
  module.exports = function ($rootScope,
    ...
    function link(scope, element, attrs) {
          ...
          if (dropData.isUpload) {
            ...
            if (dropData.filesAreValidForUploading) {
              WorkPackageAttachmentsService.upload(scope.workPackage, dropData.files).then(
                  WorkPackageAttachmentsService.load(scope.workPackage, true).then(function (attachments) {
                    ...
                    if (dropData.filesCount == 1) {
                      ...
                      if (currentFile.isAnImage) {
                        ...
                        var userInput = new UserInputModel({
                          modalTitle: "Insert Image-Attachment",
                          insertAsAlternative: "attachment"
                        });

                        userInput.showModal(function (inputResult) {

                          switch (inputResult.insertMode) {
                            case "inline":
                    ...
                    // all attachments will become inserted as attachment link
                    else {
            ...
            if(dropData.isWebImage()){
              ...
              if(dropData.isAttachmentOfCurrentWp()){
                // it's an attachment-link, so no need for the full
                // host information in the URL
                ...
                modalOptions = {
                  modalTitle: "Insert Web-Image",
                  insertAsAlternative: "link"
                };
              }

              var userInput = new UserInputModel(modalOptions);
              userInput.showModal(function (inputResult) {
                editor.insertWebLink(insertUrl,inputResult.insertMode);
              });
            }
            else{
      ...
      function UserInputModel(options){

        var callbackFunction;

        /**
         * @desc sets the options for the modal and activates it.
         * @param {function} callback - method that will be executed after the modal has been closed.
           */
        this.showModal = function(callback){
          attachmentsSimpleModal.options = options;
          attachmentsSimpleModal.modal.activate();
          callbackFunction = callback;
        };

        /**
         * @desc invokes the callback from showModal() and cleans up our modal instance after it has been deactivated.
         * @type {Function|*|(function())}
           */
        var removeEventListener = scope.$on("AttachmentsSimpleModalClosed", function(){
          callbackFunction(attachmentsSimpleModal.data);
          removeEventListener();
        });

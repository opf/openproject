...
export class DropModel{
  ...
  constructor(protected $location: ng.ILocationService, protected dt: DataTransfer, protected workPackage: WorkPackageResource){
    ...
    this.isUpload = this.isUpload(dt);
  ...
  public isAttachmentOfCurrentWp():boolean {
    const attachments = this.workPackage.attachments.elements;

    if(this.isWebLink){

      if(!(this.webLinkUrl.indexOf(this.$location.host()) > -1) ) return false;

      return angular.isDefined(attachments.filter(attachment=>{
        return this.webLinkUrl.indexOf(attachment.downloadLocation.$link.href) > -1;
      })[0]);
  ...
  protected isUpload(dt: DataTransfer): boolean {
    var isUpload = false;

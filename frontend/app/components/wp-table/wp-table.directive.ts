...
function wpTableDirective($window,
                          $state,
                          $rootScope,
                          ...
                          QueryService,
                          PathHelper,
                          apiWorkPackages): IDirective {

  function wpTableDirectiveLink(scope: any, element) {
    ...
    function mulipleRowsChecked() {
      ...
      for (var i = 0, l = scope.rows.length; i < l; i++) {
        if (scope.rows[i].checked && (++counter === 2)) {
          return true;
        }
      }
      return false;
  ...
  return {
    ...
    link: wpTableDirectiveLink
  };
}

wpDirectivesModule.directive('wpTable', wpTableDirective);

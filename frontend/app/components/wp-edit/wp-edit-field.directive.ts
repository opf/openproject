...
import {WorkPackageEditFieldFactory} from "./wp-edit-field.module";
...
export class WorkPackageEditFieldController {
  ...
  public activate() {
    this.setupField().then(() => {
      this._active = this.field.writable && true;
    });
  }
  ...
  protected setupField():ng.IPromise {
    return this.formCtrl.loadSchema().then(() =>  {
      this.field = WorkPackageEditFieldFactory.create(this.workPackage, this.fieldName);
    });
  }
}

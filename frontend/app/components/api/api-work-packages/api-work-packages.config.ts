...
function apiWorkPackagesConfig(RestangularProvider) {
  RestangularProvider.addElementTransformer('work_packages', workPackage => {
     ...
     *
     * We want the added properties to not be enumerable.
     */
    Object.defineProperties(workPackage, {
       ...
       *
       * The linked schema offers no information about allowed values
       * for fields with selectable values, so we have to use the form's
       * embedded schema.
       *
       * As the schema is also a linked property, we add a schema attribute to
       * the work package.
       *
       * @method
       */
      //TODO: Return the schema immediately, if it has already been loaded.
      getSchema: {
        value: () => workPackage.getForm().then(form => {
          //TODO: Don't set schema property
          Object.defineProperty(workPackage, 'schema', {value: form.embedded.schema});

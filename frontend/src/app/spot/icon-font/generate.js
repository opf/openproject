...
const allIconNames = filesBoxed.map(
  (f) => f
    .replace(path.join(TEMPLATE_DIR, 'icons'), '')
    .replace('.svg', '')
    .replace('/', '')
);

fs.writeFileSync(path.join(__dirname, 'icons.json'), JSON.stringify(allIconNames, null, 2));

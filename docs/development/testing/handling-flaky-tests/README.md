...
An assertion running between steps 1 and 4 sees stale content and fails intermittently. The helpers below (in `spec/support/shared/cuprite_helpers.rb` and `spec/support/toasts/expectations.rb`) register their JS event listeners _before_ yielding the block, so they are race-condition-safe. Wrap only the triggering action — never the assertion.

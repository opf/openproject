...
Handling locked users is also done from the list. To disable a users access click the **Lock permanently** link next to a user. Use the **Unlock** link to restore the users access.

If you are using [Enterprise cloud](../../../cloud-edition-guide) or [Enterprise on-premises](../../../enterprise-edition-guide) you will then have a new user available to add to the system within your booked plan, or will consume a user from that plan.

...
Click on *Create* to finish the creation of the synchronized filter. This filter is executed hourly as part of the background job before the actual group synchronization runs.
...
1. The connection, or the LDAP group needs to have *Sync users* checked. This setting overrides the LDAP connection's *Sync users* attribute for fine-grained control over which groups will have users synchronized.

...
To configure any of these settings, navigate to your user name and select -> _Administration_ -> _Authentication_.
...
- **How users sign in to OpenProject.** Users can either log in with an account managed inside OpenProject, or OpenProject can delegate authentication to a central system you already operate (a directory or an SSO identity provider). Before starting to invite more users to the system, this is the most central question administrators need to plan for.
...
Use the following table as a starting point to choose how your users should sign in. You can also combine several methods or authentication providers (for example internal accounts for external guests alongside SSO for employees).

| Your situation                                               | Recommended approach                                         | Edition           |
| ------------------------------------------------------------ | ------------------------------------------------------------ | ----------------- |
...
| You use an **OpenID Connect identity provider** (Microsoft Entra ID, Keycloak, Google Workspace, Okta, ...) | [OpenID Connect single sign-on](openid-providers)            | Enterprise add-on |
| Your identity provider is **SAML-based** (e.g. ADFS, Shibboleth) | [SAML single sign-on](saml)                                  | Enterprise add-on |

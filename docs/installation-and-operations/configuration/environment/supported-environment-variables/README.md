...
- `<randomly generated>` — a random secret, created once when it is first used.
- `<depends on environment>` — the default differs between the
...
OPENPROJECT_COLLABORATIVE__EDITING__HOCUSPOCUS__SECRET (default=<depends on environment>) The secret used for generating access tokens to access documents on hocuspocus server.
OPENPROJECT_COLLABORATIVE__EDITING__HOCUSPOCUS__URL (default=<depends on environment>) The URL of the hocuspocus server used by BlockNoteJS editor to enable collaborative editing.
...
OPENPROJECT_DEFAULT__PROJECTS__MODULES (default=<depends on environment>) Default enabled modules for new projects
...
OPENPROJECT_DEVELOPMENT__HIGHLIGHT__ENABLED (default=<depends on environment>) Enable highlighting of development environment
...
OPENPROJECT_FEATURE__BUILT__IN__OAUTH__APPLICATIONS__ACTIVE (default=<depends on environment>) Allows the display and use of built-in OAuth applications.
OPENPROJECT_FEATURE__DEPLOY__TARGETS__ACTIVE (default=<depends on environment>)
OPENPROJECT_FEATURE__MINUTES__STYLING__MEETING__PDF__ACTIVE (default=<depends on environment>) Allow exporting a meeting with FITKO styling. See #65124 for details.
OPENPROJECT_FEATURE__SPRINT__REPORTS__ACTIVE (default=<depends on environment>) Enables sprint reporting within the backlogs module. It shows a dashboard with various widgets regarding the sprint progress.
OPENPROJECT_FEATURE__STORAGE__FILE__PICKING__SELECT__ALL__ACTIVE (default=<depends on environment>)
OPENPROJECT_FEATURE__TYPE__VARIANTS__ACTIVE (default=<depends on environment>) Enables work package type variants.
OPENPROJECT_FEATURE__WIKI__ENHANCEMENTS__ACTIVE (default=<depends on environment>) Enables Wiki enhancements, such as the Wikis tab and XWiki integration.
...
OPENPROJECT_HOST__NAME (default=<depends on environment>) Host name
...
OPENPROJECT_HTTPS (default=<depends on environment>) Set assumed connection security for the Rails processes
...
OPENPROJECT_LOG__LEVEL (default=<depends on environment>) Set the OpenProject logger level
...
OPENPROJECT_LOOKBOOK__ENABLED (default=<depends on environment>) Enable the Lookbook component documentation tool. Discouraged for production environments.
...
OPENPROJECT_PASSWORD__ACTIVE__RULES (default=<depends on environment>) Password requirements
...
OPENPROJECT_PLUGIN__OPENPROJECT__AVATARS (default=<depends on environment>)
...
OPENPROJECT_REAL__TIME__TEXT__COLLABORATION__ENABLED (default=<depends on environment>) Enable real-time collaborative editing of text fields using BlockNoteJS and Hocuspocus server.
...
OPENPROJECT_SHOW__WARNING__BARS (default=<depends on environment>) Render warning bars (pending migrations, deprecation, unsupported browsers)
...
OPENPROJECT_WEB (default=<depends on environment>) Web worker count and threads configuration

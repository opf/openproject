...
- [ ] **Audit existing OpenProject custom fields** for name collisions with fields you expect to import. A same-name field of the same format is reused rather than duplicated, and for List and Hierarchy fields that happens only when the existing field's options already cover every value the import needs — otherwise you get an auto-renamed extra field (e.g., My Field (2)).
...
- [ ] If Labels-type custom fields are in use, be aware the migrator can only recover option values that actually appear on an imported issue — Jira exposes no allowed values for them, so values that exist in your data but on no imported issue won't come across as selectable options. If you want them migrated to OpenProject, add the values at least temporarily to at least one Jira issue. Option-based fields (select, radio button, multi-select, checkbox, cascading select) are not affected: their full option set comes across, used or not. The exception is Jira Data Center older than 9.3, where the options have to be read off an edit screen — there, a field on no edit screen reports no options and the same "only what an issue uses" limitation applies.
...
- [ ] Standardize Status's categories (i.e. "To Do", "In Progress", "Done") across all projects. Whether a status counts as "closed" in OpenProject is taken from Jira's status category the first time that status is created. If a later project has the same name with a different category, it is not changed again. If this applies to you, either rename one of them in Jira, assign them to the same status category or adapt the resulting `is_closed` value manually afterward.
...
- [ ] **Group projects that share custom field options into the same batch.** Contexts are grouped per run, so importing such projects together is what merges their differing option sets and settles the field naming in one go — see the custom fields clean-up item above. Projects that share identical options are safe to split: a later batch reuses the field an earlier one created instead of adding a duplicate.

...
- [ ] **Enable project-based semantic identifiers** in OpenProject *before* trying to configure the Jira import — the configuration button stays disabled otherwise.
- [ ] **Disable "Set start date to creation date" setting** in OpenProject. This is currently needed due to a [known issue](https://community.openproject.org/projects/JIM/work_packages/JIM-155/activity) in the importer.
...
- [ ] Check for user references that may no longer resolve in Jira — for example, comment authors, attachment authors, changelog authors, or assignees who have since been removed from Jira. This is not handled gracefully: any of these failing to resolve aborts the whole import, even on a clean "not found" from Jira. Confirmed, currently open bug: [JIM-184](https://community.openproject.org/projects/JIM/work_packages/JIM-184). @mentions inside descriptions/comments are the one exception — they fall back to plain text instead of failing. Worth testing your actual data for this rather than assuming it's fine. Re-create the users in Jira, create them before the import manually in OpenProject or remove the reference from the Jira issue if possible.
- [ ] Check that users referenced in user picker custom fields are members of a project you're migrating. If someone is only referenced this way (e.g. as an assignee-style custom field value) but isn't a member of any project in scope, that value will silently come across empty. Add them as a member of the relevant Jira project before migrating to avoid this.
- [ ] Consolidate custom field contexts for select-type and cascading-select fields. If a Jira custom field has different allowed values configured per project or issue type (different "contexts"), each distinct set becomes its own separate OpenProject field. Aligning these into one consistent set of options in Jira beforehand avoids ending up with several OpenProject fields for what's really one field. Note that this only helps within a single import run. If you're migrating in separate batches, this same field will still get a new, separate OpenProject field in each batch, even with identical options. If you don’t want this behavior, design the batches accordingly. Custom fields with non-ASCII characters (e.g. accented letters) in the name have also been confirmed to duplicate across separate runs: [JIM-170](https://community.openproject.org/projects/JIM/work_packages/JIM-170).

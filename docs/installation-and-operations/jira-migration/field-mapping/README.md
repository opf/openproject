...
| Remaining time estimate                            | Remaining hours           | ⚠️ Same 60× inflation as Estimated hours above — confirmed via testing (e.g. `2d` in Jira → `960` in OpenProject instead of `16`). Tracked as [JIM-185](https://community.openproject.org/projects/JIM/work_packages/JIM-185) |
...
- **Watchers are never migrated**, in any form.
...
- **Time estimates come across 60× too high, confirmed by testing.** The code divides Jira's raw seconds value by 60 (giving minutes) instead of 3600 (giving hours). Concretely: entering `1d` (Jira's default is 8 working hours) results in `480` in OpenProject's Estimated hours field — 60 times the correct value of `8`. Divide any migrated estimate/remaining-hours value by 60 to get the real number. This is a confirmed, currently open bug: [JIM-185](https://community.openproject.org/projects/JIM/work_packages/JIM-185).
...
- **A failed import run rolls back entirely** — if something goes wrong partway through, nothing from that run is kept, not even the projects that succeeded before the failure.

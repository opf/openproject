...
| Logged work / time entries                         | **Not migrated**          | - (planned: [JIM-93](https://community.openproject.org/projects/JIM/work_packages/JIM-93)) |
| Original time estimate                             | Work                      | -                                                            |
...
| Remaining time estimate                            | Remaining work            | -                                                            |
...
| Status                                             | Status                    | Matched by name (not case-sensitive); created automatically if missing. `is_closed` is set depending on Jira's status category ("Done" maps to a closed status in OpenProject) on first creation. Note that this only happens the *first* time a status with that name is created. If the same status name already exists (e.g. reused across projects with inconsistent status categories), it is reused and the existing `is_closed` value is kept. |

...
- [ ] **Double-check migrated statuses are correctly flagged as closed.** The "closed" value of an OpenProject status is set automatically from Jira's status category the *first* time a status is created. If the same status name is used across multiple Jira projects with inconsistent status categories, whichever project's issues get migrated first determines the result for all of them. Spot-check via Administration → Work packages → Status and adapt manually if needed. 
...
- [ ] **Fill in the remaining details for every newly-created Type, Status, and Priority** as needed.
...
- [ ] **Check the custom fields you don't recognise from the Jira issue view.** A company-wide Jira field can hold values on an issue without appearing on that project's screens, so it is imported and activated on the projects whose issues carry it. Decide per field whether to keep it, and remove the rest from those projects - see [Custom fields you see in OpenProject but not on the Jira issue](../custom-fields/#custom-fields-you-see-in-openproject-but-not-on-the-jira-issue).

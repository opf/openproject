...
Then, go into the compose folder, all further commands must be executed inside that folder:
...
You can stop the Compose stack if you want to keep the data by running:

```
docker-compose stop
```

You can stop and remove the Compose stack including its data by running:

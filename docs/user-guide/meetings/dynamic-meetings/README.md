...
You can add sections or items to an agenda or directly link to existing work packages by selecting the desired option under the green **Add** button. You can add notes and [attachments](#meeting-attachments) to every agenda item.
...
You can then add agenda items to specific section.

![Add an agenda item to a meeting section](openproject_dynamic_meetings_add_item_to_section.png)

 If you use the green **+Add** button on the left and there are multiple sections in place, the item will be added to the section that was created last.

Section will show the sum of all the items duration. 

![Duration of a section in OpenProject meeting](openproject_dynamic_meetings_section_duration.png)

You can edit a section, move it, add notes or delete by selecting the respective option from the dropdown menu behind the **More** (three dots) icon on the right side. If the section is not empty, i.e it includes agenda items and/or work packages, you will be asked for confirmation before deletion.

![Edit or delete a section in an OpenProject meeting](openproject_dynamic_meetings_edit_section_options.png)

You can also re-arrange sections by dragging and dropping. If a section is moved, the agenda items will move along with it. 

#### Add an agenda item
If you select the **Agenda item** option, you can name that item, add notes, set the anticipated duration in minutes and select a user to be displayed next to the agenda item. This could for example be a meeting or a project member that is accountable for or a presenter of this particular agenda item. By default the name of the user adding the agenda item will be pre-selected, but it can either be removed or replaced by one of the other meeting invitees.

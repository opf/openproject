  ...
  describe "PATCH /api/v3/work_packages/:id" do
    ...
    context "user with needed permissions" do
      ...
      describe "observedInVersions" do
        let(:observed_version) { create(:version, project:) }
        let(:observed_versions_links) { [{ href: api_v3_paths.version(observed_version.id) }] }
        let(:params) { valid_params.merge(_links: { observedInVersions: observed_versions_links }) }
        ...
        context "with a single version" do
          ...
          it "assigns the observed in version" do
            expect(work_package.reload.observed_in_versions).to contain_exactly(observed_version)
          end
        ...
        context "with multiple versions" do
          let(:other_version) { create(:version, project:) }
          let(:observed_versions_links) do
            [{ href: api_v3_paths.version(observed_version.id) },
             { href: api_v3_paths.version(other_version.id) }]
          end
          ...
          it "assigns all observed in versions" do
            expect(work_package.reload.observed_in_versions)
              .to contain_exactly(observed_version, other_version)
          end
        end
        ...
        context "with an empty collection" do
          ...
          let(:observed_versions_links) { [] }

          include_context "patch request"

          it { expect(response).to have_http_status(:ok) }

          it "clears the observed in versions" do
            expect(work_package.reload.observed_in_versions).to be_empty
          end
        end

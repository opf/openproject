...
describe ::API::V3::Events::EventsAPI,
  ...
  let(:send_read) do
    ...
    post api_v3_paths.event_read_ian(event.id)
  end

  let(:send_unread) do
    header "Content-Type", "application/json"
    post api_v3_paths.event_unread_ian(event.id)
  end
  ...
  describe 'recipient user' do
    ...
    it 'can read and unread' do
      ...
      expect(event.reload.read_ian).to be_truthy

      send_unread
      expect(last_response.status).to eq(204)
      expect(event.reload.read_ian).to be_falsey
    end
  end

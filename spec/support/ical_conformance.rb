...
class ICalConformance
  ...
  def count_and_until_violations
    event_rrules.filter_map do |event, recur|
  ...
  def build_schedule(master)
    ...
    expand(master, recur.value_ical)
  end

  def expand(master, rrule)
    IceCube::Schedule
      .new(local_time(master.dtstart))
      .tap { it.add_recurrence_rule(IceCube::Rule.from_ical(rrule)) }
  rescue StandardError => e
    "the RRULE #{rrule} of #{describe(master)} could not be expanded: #{e.message}"
  end

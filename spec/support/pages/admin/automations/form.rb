      ...
      class Form < ::Pages::Page
        ...
        def add_action(name, value)
          retry_block do
            ...
            within "#automations-form--active-actions" do
        ...
        def expect_action(name, value)
          ...
          within "#automations-form--actions" do
            if value.is_a?(Array)
              value.each { |name| expect_selected_option(name.to_s) }
            else
              element = find("input[name='automation[actions][#{name}]']", visible: :all)
        ...
        def set_condition(name, value)
          Array(value).each do |val|
            retry_block do
              ...
              within "#automations-form--conditions" do
        ...
        def set_action_value(name, value)
          field = find("#automations-form--active-actions .form--field", text: name, wait: 5)

          set_field_value(field, name, value)
        end

        def set_condition_value(name, value)
          field = find("#automations-form--conditions .form--field", text: name, wait: 5)

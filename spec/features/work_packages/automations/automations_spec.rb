...
RSpec.describe "Automations", :js, with_ee: %i[custom_actions] do
  ...
  it "viewing workflow buttons" do
    ...
    new_ca_page.expect_trigger_options("Button press")
    ...
    new_ca_page.set_button_label("Close")
    ...
    new_ca_page.set_button_label("Escalate")
    ...
    new_ca_page.set_button_label("Reset")
    ...
    new_ca_page.set_button_label("Other roles action")
    ...
    retry_block do
      ...
      new_ca_page.set_button_label("Move project")
    ...
    wp_page.expect_attributes assignee: "-",
                              ...
                              "customField#{date_custom_field.id}" => (Time.zone.today + 5.days).strftime("%m/%d/%Y")
    expect(page)
      .to have_text(I18n.t("js.project.click_to_switch_to_project", projectname: other_project.name))
  ...
  it "editing a current date custom action (Regression #30949)" do
    ...
    new_ca_page.set_button_label("Current date")
  ...
  it "editing a status custom action (Regression #61888)" do
    ...
    new_ca_page.set_button_label("Status")
  ...
  context "with a multi_user field (Bug#64981)" do
    it "saves when a user is assigned to the custom field" do
      ...
      new_ca_page.set_button_label("MultiUser")
  ...
  context "when staying on the page for multiple changes (Bug#OP-19472)",
    ...
    it "works when clicking two buttons" do
      ...
      wp_page.click_automation(unassign_ca.name, expect_success: true)

      wp_page.click_automation(responsible_ca.name, expect_success: true)

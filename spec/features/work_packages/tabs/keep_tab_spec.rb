...
RSpec.feature 'Keep current details tab', js: true, selenium: true do
  ...
  scenario 'Remembers the tab while navigating the page' do
    ...
    wp_split = wp_table.open_split_view(wp1)
    wp_split.expect_subject
    wp_split.visit_tab! :activity

    wp_split = wp_table.open_split_view(wp2)
    wp_split.expect_subject
    wp_split.expect_tab :activity
    wp_split.visit_tab! :relations
    ...
    wp_full = wp_split.switch_to_fullscreen

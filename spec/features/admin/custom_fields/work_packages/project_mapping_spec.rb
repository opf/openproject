...
RSpec.describe "Custom Fields Multi-Project Activation", :js do
  ...
  let(:custom_field_projects_mappings_page) { Pages::CustomFields::ProjectMappings.new }
  ...
  context "with sufficient permissions" do
    ...
    it "allows unlinking a project from a custom field", with_settings: { per_page_options: "2,5" } do
      ...
      aggregate_failures "pagination links maintain the correct url after unlinking is done" do
        custom_field_projects_mappings_page.expect_page_links(model: custom_field, current_page:)
        custom_field_projects_mappings_page.expect_current_page_number(current_page)
      end
    end

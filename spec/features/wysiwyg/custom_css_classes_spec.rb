...
describe 'Wysiwyg paragraphs in lists behavior (Regression #28765)',
  ...
  it 'custom classes are placed correctly' do
    editor.in_editor do |container, editable|
      ...
      expect(editable).to have_css('figure.op-uc-figure_align-center', count: 1)
      expect(editable).to have_css('img.op-uc-image', count: 1)
      expect(editable).to have_css('table.op-uc-table', count: 1)
      expect(editable).to have_css('div.op-uc-figure--content', count: 2)
      expect(editable).to have_css('thead.op-uc-table--head', count: 1)
      expect(editable).to have_css('tr.op-uc-table--row', count: 2)
      expect(editable).to have_css('td.op-uc-table--cell', count: 2)
      expect(editable).to have_css('th.op-uc-table--cell.op-uc-table--cell_head', count: 4)
      ...
      expect(editable).to have_css('li.op-uc-list--item', count: 3)

...
describe 'timelines/colors/index.api.rsb' do
  before do
  ...
  describe 'with 3 colors available' do
    ...
    it 'renders the _color template for each assigned color' do
      ...
      view.should_receive(:render).exactly(3).times.with(hash_including(:partial => '/timelines/colors/color.api')).and_return('')

      # just to render the speced template despite the should receive expectations above
      view.should_receive(:render).once.with({:template=>"timelines/colors/index", :handlers=>["rsb"], :formats=>["api"]}, {}).and_call_original

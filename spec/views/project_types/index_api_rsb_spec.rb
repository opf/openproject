...
describe 'timelines/project_types/index.api.rsb' do
  before do
  ...
  describe 'with 3 project types available' do
    ...
    it 'renders the _project_type template for each assigned project type' do
      ...
      view.should_receive(:render).exactly(3).times.with(hash_including(:partial => '/timelines/project_types/project_type.api')).and_return('')

      # just to call the original render method
      view.should_receive(:render).once.with({:template=>"timelines/project_types/index", :handlers=>["rsb"], :formats=>["api"]}, {}).and_call_original

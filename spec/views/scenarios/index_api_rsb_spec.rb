...
describe 'timelines/scenarios/index.api.rsb' do
  before do
  ...
  describe 'with 3 scenarios available' do
    ...
    it 'renders the _scenario template for each assigned scenario' do
      ...
      view.should_receive(:render).exactly(3).times.with(hash_including(:partial => '/timelines/scenarios/scenario.api')).and_return('')

      # just to call the speced template despite the should receive expectations above
      view.should_receive(:render).once.with({ :template=>"timelines/scenarios/index", :handlers=>["rsb"], :formats=>["api"] }, {}).and_call_original

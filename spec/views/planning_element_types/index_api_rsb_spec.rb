...
describe 'timelines/planning_element_types/index.api.rsb' do
  before do
  ...
  describe 'with 3 planning element types available' do
    let(:planning_element_types) do
      [
        FactoryGirl.build(:planning_element_type),
        FactoryGirl.build(:planning_element_type),
        FactoryGirl.build(:planning_element_type)
      ]
    end
    ...
    it 'renders the _planning_element_type template for each assigned planning element type' do
      ...
      view.should_receive(:render).exactly(3).times.with(hash_including(:partial => '/timelines/planning_element_types/planning_element_type.api')).and_return('')

      # just to render the speced template despite the should receive expectations above
      view.should_receive(:render).once.with({:template=>"timelines/planning_element_types/index", :handlers=>["rsb"], :formats=>["api"]}, {}).and_call_original

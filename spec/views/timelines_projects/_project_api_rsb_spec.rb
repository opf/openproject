...
describe 'timelines/timelines_projects/_project.api' do
  before do
  ...
  def render
    params[:format] = 'xml'
    super(:partial => 'timelines/timelines_projects/project.api', :object => project)
  end
  ...
  describe 'with a project having a responsible' do
    ...
    let(:project) { FactoryGirl.create(:project,
                                   :timelines_responsible_id => responsible.id) }
  ...
  describe 'with a project having a project type' do
    let(:project_type) { FactoryGirl.build(:timelines_project_type, :id => 100, :name => 'Awesometastic') }

    let(:project) { FactoryGirl.build(:project, :timelines_project_type_id => project_type.tap { |p| p.save! }.id) }
  ...
  describe 'with a project having 3 enabled planning element types' do
    let(:color)        { FactoryGirl.create(:timelines_color) }
    let(:project)      { FactoryGirl.create(:project) }

    before do
      types = [
        FactoryGirl.create(:timelines_planning_element_type, :color_id => color.id),
        FactoryGirl.create(:timelines_planning_element_type, :color_id => color.id),
        FactoryGirl.create(:timelines_planning_element_type, :color_id => color.id)
      ]
      types.each do |type|
  ...
  describe 'with a project having project_associations' do
    ...
    before do
      @project_scenarios = [
        FactoryGirl.create(:timelines_project_association,

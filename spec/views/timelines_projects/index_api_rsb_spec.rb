...
describe '/timelines/timelines_projects/index.api.rsb' do
  before do
  ...
  describe 'with some projects available' do
    ...
    it 'renders the _project template for each assigned project' do
      ...
      view.should_receive(:render).exactly(3).times.with(hash_including(:partial => '/timelines/timelines_projects/project.api')).and_return('')

      # just to call the speced template despite the should receive expectations above
      view.should_receive(:render).once.with({ :template=>"/timelines/timelines_projects/index", :handlers=>["rsb"], :formats=>["api"] }, {}).and_call_original

...
describe 'timelines/planning_element_statuses/show.api.rsb' do
  before do
  ...
  describe 'with an assigned planning element status' do
    ...
    it 'renders the _planning_element_status template once' do
      ...
      view.should_receive(:render).once.with(hash_including(:partial => '/timelines/planning_element_statuses/planning_element_status.api')).and_return('')

      # just to render the speced template despite the should receive expectations above
      view.should_receive(:render).once.with({:template=>"timelines/planning_element_statuses/show", :handlers=>["rsb"], :formats=>["api"]}, {}).and_call_original

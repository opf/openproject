...
describe 'timelines/timelines_planning_element_types/show.api.rsb' do
  before do
  ...
  describe 'with an assigned planning element type' do
    ...
    it 'renders the _planning_element_type template once' do
      ...
      view.should_receive(:render).once.with(hash_including(:partial => '/timelines/timelines_planning_element_types/planning_element_type.api')).and_return('')

      # just to render the speced template despite the should receive expectations above
      view.should_receive(:render).once.with({:template=>"timelines/timelines_planning_element_types/show", :handlers=>["rsb"], :formats=>["api"]}, {}).and_call_original

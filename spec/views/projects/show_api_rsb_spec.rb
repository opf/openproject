...
describe 'timelines/projects/show.api.rsb' do
  before do
  ...
  describe 'with an assigned project' do
    ...
    it 'renders the _project view once' do
      ...
      view.should_receive(:render).once.with(hash_including(:partial => '/timelines/projects/project.api')).and_return('')

      # just to render the speced template despite the should receive expectations above
      view.should_receive(:render).once.with({:template=>"timelines/projects/show", :handlers=>["rsb"], :formats=>["api"]}, {}).and_call_original

...
describe '/timelines/projects/index.api.rsb' do
  before do
  ...
  describe 'with some projects available' do
    ...
    it 'renders the _project template for each assigned project' do
      ...
      view.should_receive(:render).exactly(3).times.with(hash_including(:partial => '/timelines/projects/project.api')).and_return('')

      # just to call the speced template despite the should receive expectations above
      view.should_receive(:render).once.with({ :template=>"/timelines/projects/index", :handlers=>["rsb"], :formats=>["api"] }, {}).and_call_original

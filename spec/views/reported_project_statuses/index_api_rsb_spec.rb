...
describe 'timelines/reported_project_statuses/index.api.rsb' do
  before do
  ...
  describe 'with 3 reported_project_statuses available' do
    ...
    it 'renders the _reported_project_status template for each assigned reported_project_status' do
      ...
      view.should_receive(:render).exactly(3).times.with(hash_including(:partial => '/timelines/reported_project_statuses/reported_project_status.api')).and_return('')

      # just to render the speced template despite the should receive expectations above
      view.should_receive(:render).once.with({:template=>"timelines/reported_project_statuses/index", :handlers=>["rsb"], :formats=>["api"]}, {}).and_call_original

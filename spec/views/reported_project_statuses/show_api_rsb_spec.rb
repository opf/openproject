...
describe 'timelines/reported_project_statuses/show.api.rsb' do
  before do
  ...
  describe 'with an assigned reported_project_status' do
    ...
    it 'renders the _reported_project_status template once' do
      ...
      view.should_receive(:render).once.with(hash_including(:partial => '/timelines/reported_project_statuses/reported_project_status.api')).and_return('')

      # just to call the original render despite the should_receive expectation
      view.should_receive(:render).once.with({:template=>"timelines/reported_project_statuses/show", :handlers=>["rsb"], :formats=>["api"]}, {}).and_call_original

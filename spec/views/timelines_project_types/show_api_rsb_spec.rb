...
describe 'timelines/timelines_project_types/show.api.rsb' do
  before do
  ...
  describe 'with an assigned project type' do
    let(:project_type) { FactoryGirl.build(:timelines_project_type) }
    ...
    it 'renders the _project_type template once' do
      ...
      view.should_receive(:render).once.with(hash_including(:partial => '/timelines/timelines_project_types/project_type.api')).and_return('')

      # just to be able to render the speced template despite the should receive expectation above
      view.should_receive(:render).once.with({ :template=>"timelines/timelines_project_types/show",
                                               :handlers=>["rsb"],
                                               :formats=>["api"] },
                                             {})
                                  .and_call_original

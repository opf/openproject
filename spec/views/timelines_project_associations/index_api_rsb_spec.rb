...
describe 'timelines/timelines_project_associations/index.api.rsb' do
  before do
  ...
  describe 'with 3 project_associations available' do
    let(:project_associations) do
      [
        FactoryGirl.build(:timelines_project_association),
        FactoryGirl.build(:timelines_project_association),
        FactoryGirl.build(:timelines_project_association)
      ]
    end
    ...
    it 'renders the _project_association template for each assigned project_association' do
      ...
      view.should_receive(:render).exactly(3).times.with(hash_including(:partial => '/timelines/timelines_project_associations/project_association.api')).and_return('')

      # just to render the speced template despite the should receive expectations above
      view.should_receive(:render).once.with({:template=>"timelines/timelines_project_associations/index", :handlers=>["rsb"], :formats=>["api"]}, {}).and_call_original

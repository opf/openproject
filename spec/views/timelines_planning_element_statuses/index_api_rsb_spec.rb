...
describe 'timelines/timelines_planning_element_statuses/index.api.rsb' do
  before do
  ...
  describe 'with 3 planning element statuses available' do
    let(:planning_element_statuses) do
      [
        FactoryGirl.build(:timelines_planning_element_status),
        FactoryGirl.build(:timelines_planning_element_status),
        FactoryGirl.build(:timelines_planning_element_status)
      ]
    end
    ...
    it 'renders the _planning_element_status template for each assigned planning element status' do
      ...
      view.should_receive(:render).exactly(3).times.with(hash_including(:partial => '/timelines/timelines_planning_element_statuses/planning_element_status.api')).and_return('')

      # just to be able to call render despite the should_receive expectations above
      view.should_receive(:render).once.with({:template=>"timelines/timelines_planning_element_statuses/index", :handlers=>["rsb"], :formats=>["api"]}, {}).and_call_original

...
describe 'timelines/reportings/show.api.rsb' do
  before do
  ...
  describe 'with an assigned reporting' do
    ...
    it 'renders the _reporting template once' do
      ...
      view.should_receive(:render).once.with(hash_including(:partial => '/timelines/reportings/reporting.api')).and_return('')

      # just to render the speced template despite the should receive expectations above
      view.should_receive(:render).once.with({:template=>"timelines/reportings/show", :handlers=>["rsb"], :formats=>["api"]}, {}).and_call_original

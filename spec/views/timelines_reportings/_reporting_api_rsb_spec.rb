...
describe 'timelines/timelines_reportings/_reporting.api' do
  before do
  ...
  def render
    params[:format] = 'xml'
    super(:partial => 'timelines/timelines_reportings/reporting.api', :object => reporting)
  end
  ...
  describe 'with an assigned reporting' do
    ...
    describe 'reporting node with reported_project_status' do
      let(:reported_project_status) { FactoryGirl.create(:timelines_reported_project_status,
                                                     :id => 1,
                                                     :name => 'beste') }
      let(:reporting) { FactoryGirl.build(:timelines_reporting,

...
describe 'timelines/timelines_scenarios/show.api.rsb' do
  before do
  ...
  describe 'with an assigned scenario' do
    ...
    it 'renders the _scenario template once' do
      view.should_receive(:render).once.with(hash_including(:partial => '/timelines/timelines_scenarios/scenario.api')).and_return('')

      # just to render the speced template despite the should receive expectations above
      view.should_receive(:render).once.with({:template=>"timelines/timelines_scenarios/show", :handlers=>["rsb"], :formats=>["api"]}, {}).and_call_original

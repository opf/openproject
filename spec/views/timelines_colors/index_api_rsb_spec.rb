...
describe 'timelines/timelines_colors/index.api.rsb' do
  before do
  ...
  describe 'with 3 colors available' do
    let(:colors) {
      [
        FactoryGirl.build(:timelines_color),
        FactoryGirl.build(:timelines_color),
        FactoryGirl.build(:timelines_color)
      ]
    }
    ...
    it 'renders the _color template for each assigned color' do
      ...
      view.should_receive(:render).exactly(3).times.with(hash_including(:partial => '/timelines/timelines_colors/color.api')).and_return('')

      # just to render the speced template despite the should receive expectations above
      view.should_receive(:render).once.with({:template=>"timelines/timelines_colors/index", :handlers=>["rsb"], :formats=>["api"]}, {}).and_call_original

...
describe 'timelines/timelines_colors/show.api.rsb' do
  before do
  ...
  describe 'with an assigned color' do
    let(:color) { FactoryGirl.build(:timelines_color) }
    ...
    it 'renders the _color template once' do
      ...
      view.should_receive(:render).once.with(hash_including(:partial => '/timelines/timelines_colors/color.api')).and_return('')

      # in order to enable calling the original render method
      # despite should_receive expectations
      view.should_receive(:render).once.with(hash_including(:template => "timelines/timelines_colors/show"), {})
                                  .and_call_original

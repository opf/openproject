...
shared_examples_for 'has ISO 8601 hours only' do
  it 'exists' do
    expect(subject).to have_json_path(json_path)
  end
  ...
  it 'indicates date and time as ISO 8601' do
    called_with_expected = false
    expect(::API::V3::Utilities::DateTimeFormatter).to receive(:format_duration_from_hours) do |actual, *_|
      # ActiveSupport flaws :eql? we circumvent that by calling utc (which is equally valid)
      called_with_expected = true if actual.eql? hours
    end.at_least(:once)

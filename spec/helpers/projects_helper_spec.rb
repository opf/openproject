...
RSpec.describe ProjectsHelper do
  ...
  describe '#enabled_projects_columns_options', with_settings: { enabled_projects_columns: %w[name project_status] } do
    let(:selects) do
      [
    ...
    it 'returns the columns options' do
      expect(helper.enabled_projects_columns_options)
        .to eql([
                  ['Description', 'description', { checked: false, disabled: false }],
                  ['Name', 'name', { checked: true, disabled: true }],
                  ['Status', 'project_status', { checked: true, disabled: false }]
                ])
    end
  end
end

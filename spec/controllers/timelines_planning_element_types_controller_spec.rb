...
describe Timelines::TimelinesPlanningElementTypesController do
  ...
  def enable_type(project, type)
    FactoryGirl.create(:timelines_enabled_planning_element_type,
  ...
  describe 'with project scope' do
    ...
    describe 'index.xml' do
      ...
      describe 'with no planning element types available' do
        ...
        it 'renders the index builder template' do
          get 'index', :project_id => project.identifier, :format => 'xml'
          response.should render_template('timelines/timelines_planning_element_types/index', :formats => ["api"])
        end
      end
      ...
      describe 'with 3 planning element types available' do
        before do
          @created_planning_element_types = [
            FactoryGirl.create(:timelines_planning_element_type),
            FactoryGirl.create(:timelines_planning_element_type),
            FactoryGirl.create(:timelines_planning_element_type)
          ]
    ...
    describe 'show.xml' do
      def fetch
        @available_type = FactoryGirl.create(:timelines_planning_element_type, :id => '1337')
      ...
      describe 'with an planning element type, which is not enabled in the project' do
        before do
          FactoryGirl.create(:timelines_planning_element_type, :id => '1337')
        end
      ...
      describe 'with an available planning element type' do
        before do
          @available_planning_element_type = FactoryGirl.create(:timelines_planning_element_type,
                                                            :id => '1337')
        ...
        it 'renders the show template' do
          get 'show', :project_id => project.identifier, :id => '1337', :format => 'xml'
          response.should render_template('timelines/timelines_planning_element_types/show', :formats => ["api"])
        end
      end
    end
  end
  ...
  describe 'without project scope' do
    ...
    describe 'show.xml' do
      ...
      describe 'with an available planning element type' do
        before do
          @available_planning_element_type = FactoryGirl.create(:timelines_planning_element_type, :id => '1337')
        end
    ...
    describe 'create.html' do
      def fetch
        post 'create', :planning_element_type => FactoryGirl.build(:timelines_planning_element_type).attributes
      end
      def expect_redirect_to
        timelines_planning_element_types_path
      end
      it_should_behave_like "a controller action with require_admin"
    end

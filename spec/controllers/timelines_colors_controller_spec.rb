...
describe Timelines::TimelinesColorsController do
  ...
  describe 'index.xml' do
    ...
    describe 'with no colors available' do
      ...
      it 'renders the index builder template' do
        get 'index', :format => 'xml'
        response.should render_template('timelines/timelines_colors/index', :formats => ["api"])
      end
    end
    ...
    describe 'with some colors available' do
      before do
        @created_colors = [
          FactoryGirl.create(:timelines_color),
          FactoryGirl.create(:timelines_color),
          FactoryGirl.create(:timelines_color)
        ]
      end
  ...
  describe 'show.xml' do
    ...
    describe 'with an available color' do
      before do
        @available_color = FactoryGirl.create(:timelines_color, :id => '1337')
      end
      ...
      it 'renders the show template' do
        get 'show', :id => '1337', :format => 'xml'
        response.should render_template('timelines/timelines_colors/show', :formats => ["api"])
      end
    end
  end
  ...
  describe "create.html" do
    def fetch
      post 'create', :color => FactoryGirl.build(:timelines_color).attributes
    end
    def expect_redirect_to
      Regexp.new(timelines_colors_path)
    end
    it_should_behave_like "a controller action with require_admin"
  end

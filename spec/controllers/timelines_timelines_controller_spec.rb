...
describe Timelines::TimelinesTimelinesController do
  # ===========================================================
  # Helpers
  ...
  def self.become_member_with_all_timelines_permissions
    let(:current_user) { FactoryGirl.create(:user) }
  ...
  def self.become_member_with_view_timelines_permissions
    let(:current_user) { FactoryGirl.create(:user) }
  ...
  def self.become_member_with_edit_timelines_permissions
    let(:current_user) { FactoryGirl.create(:user) }
  ...
  def self.become_member_with_delete_timelines_permissions
    let(:current_user) { FactoryGirl.create(:user) }
  ...
  shared_examples_for 'all actions related to an existing timeline' do
    ...
    describe 'w/o a valid timelines id' do
      ...
      describe 'w/ a known project' do
        ...
        describe 'w/ the current user being a member' do
          become_member_with_all_timelines_permissions
    ...
    describe 'w/ a valid timelines id' do
      let(:project)  { FactoryGirl.create(:project, :identifier => 'test_project') }
      let(:timeline) { FactoryGirl.create(:timelines_timelines, :project_id => project.id, :name => 'b') }
    ...
    describe 'w/ a known project' do
      ...
      describe 'w/ the current user having view_timelines permissions' do
        ...
        describe 'w/ 3 timelines within the project' do
          before do
            @created_timelines = [
              FactoryGirl.create(:timelines_timelines, :project_id => project.id, :name => 'b'),
              FactoryGirl.create(:timelines_timelines, :project_id => project.id, :name => 'c'),
              FactoryGirl.create(:timelines_timelines, :project_id => project.id, :name => 'a')
            ]
          end
  ...
  describe 'new.html' do
    ...
    describe 'w/ a known project' do
      become_member_with_edit_timelines_permissions

      let(:project) { FactoryGirl.create(:project, :identifier => 'test_project') }

      it 'renders the new template' do
        fetch :project_id => project.id
        response.should render_template('timelines/timelines_timelines/new', :formats => ["html"], :layout => :base)
      end
    ...
    describe 'w/ a known project' do
      ...
      describe 'w/ proper parameters' do
        it 'create a new timelines instance' do
          ...
          project.timelines_timelines.reload
          project.timelines_timelines.first.name.should == 'bb'
        end

        it 'redirects to show' do
          fetch :project_id => project.id, :timeline => {:name => 'bb'}

          timeline = project.timelines_timelines.reload.first
          response.should redirect_to(timelines_project_timeline_path(project, timeline))
        end
      ...
      describe 'w/o proper parameters' do
        it 'does not save the new timelines instance' do
          ...
          project.timelines_timelines.reload.should be_empty
        end
  ...
  describe 'show.html' do
    ...
    describe 'w/ everything set up' do
      ...
      become_member_with_view_timelines_permissions
      ...
      it 'renders the show template' do
        fetch :project_id => project.id, :id => timeline.id
        response.should render_template('timelines/timelines_timelines/show', :formats => ["html"], :layout => :base)
      end
    end
  end
  ...
  describe 'edit.html' do
    ...
    describe 'w/ everything set up' do
      ...
      it 'renders the edit template' do
        fetch :project_id => project.id, :id => timeline.id
        response.should render_template('timelines/timelines_timelines/edit', :formats => ["html"], :layout => :base)
      end
    end
  end
  ...
  describe 'confirm_destroy.html' do
    ...
    describe 'w/ everything set up' do
      become_member_with_delete_timelines_permissions
      ...
      it 'renders the confirm_destroy action' do
        ...
        response.should render_template('timelines/timelines_timelines/confirm_destroy', :formats => ["html"], :layout => :base)
      end
    end
  end

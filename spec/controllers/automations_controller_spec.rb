...
RSpec.describe AutomationsController, with_ee: %i[custom_actions] do
  ...
  describe "#create" do
    ...
    let!(:service) do
      service = instance_double(Automations::CreateService)
  ...
  describe "#update" do
    ...
    let!(:service) do
      service = instance_double(Automations::UpdateService)

      allow(Automations::UpdateService)
        .to receive(:new)
        .with(user: admin, automation: action)
        .and_return(service)
  ...
  describe "the trigger field" do
    ...
    shared_examples_for "a trigger select" do
      it "offers every registered trigger type" do
        ...
        expect(options.map { |option| [option.text, option[:value]] })
          .to eq([["Button press", "Automations::Triggers::Button"]])
      end
    ...
    context "on an existing automation" do
      ...
      it "preselects the stored trigger type" do
        ...
        expect(selected[:value]).to eq("Automations::Triggers::Button")
      end
    end
  end
end

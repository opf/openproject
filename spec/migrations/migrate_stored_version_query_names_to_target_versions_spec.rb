...
require "spec_helper"
require Rails.root.join("db/migrate/20260831130000_migrate_stored_version_query_names_to_target_versions")

RSpec.describe MigrateStoredVersionQueryNamesToTargetVersions, type: :model do
  ...
  it "migrates a legacy select when sort_criteria is stored as NULL" do
    ...
    expect(null_sort_criteria_query.reload.read_attribute(:column_names)).to eq(%i[id target_versions])
    expect(null_sort_criteria_query.sort_criteria_before_type_cast).to be_nil
  end

  it "renames the stored select, sort, group_by, and filter names to target_versions" do
    ActiveRecord::Migration.suppress_messages { described_class.migrate(:up) }

    expect(legacy_query.reload.read_attribute(:column_names)).to eq(%i[id target_versions])
    expect(legacy_query.read_attribute(:sort_criteria)).to eq([["target_versions", "asc"], ["id", "asc"]])
    expect(legacy_query.read_attribute(:group_by)).to eq("target_versions")
    expect(parsed_filters(legacy_query)).to eq("target_version_id" => { "operator" => "=", "values" => ["1"] })
  end

  it "dedupes selects when both the legacy and the target name are already stored" do
    ActiveRecord::Migration.suppress_messages { described_class.migrate(:up) }

    expect(both_selects_dedupe_query.reload.read_attribute(:column_names)).to eq(%i[id target_versions])
    expect(both_selects_dedupe_query.read_attribute(:sort_criteria)).to eq([["target_versions", "asc"]])
  end

  it "keeps the entry already stored under the target filter key when both are present" do
    ActiveRecord::Migration.suppress_messages { described_class.migrate(:up) }
  ...
  it "keeps the target filter key's entry regardless of storage order" do
    ActiveRecord::Migration.suppress_messages { described_class.migrate(:up) }
  ...
  it "leaves a query already stored under the target names byte-identical" do

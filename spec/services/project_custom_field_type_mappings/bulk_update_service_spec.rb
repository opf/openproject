...
RSpec.describe ProjectCustomFieldTypeMappings::BulkUpdateService do
  ...
  let(:instance) { described_class.new(user:, variant:, project_custom_field_section:) }
  ...
  context "with admin permissions" do
    ...
    it "bulk enables and disables all project attributes of the section" do
      ...
      variant.project_custom_fields << other_project_custom_field

      expect(instance.call(action: :disable)).to be_success

      expect(variant.reload.project_custom_fields).to contain_exactly(other_project_custom_field)
    end

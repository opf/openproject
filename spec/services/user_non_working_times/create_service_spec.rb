...
RSpec.describe UserNonWorkingTimes::CreateService do
  ...
  let(:end_date) { Date.tomorrow + 4.days }
  let(:params) { { user: target_user, start_date:, end_date: } }
  ...
  context "when the current user has the global manage_working_times permission" do
    ...
    it "creates the non-working time record successfully" do
      expect(service_call).to be_success
      expect(service_call.result).to be_a(UserNonWorkingTime)
      expect(service_call.result).to be_persisted
      expect(service_call.result.user).to eq(target_user)
      expect(service_call.result.start_date).to eq(start_date)
      expect(service_call.result.end_date).to eq(end_date)
    end
  end
  ...
  context "when the current user has manage_own_working_times for their own record" do
    ...
    let(:params) { { user: current_user, start_date:, end_date: } }

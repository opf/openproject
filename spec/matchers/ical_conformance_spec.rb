...
RSpec.describe ICalConformance do
  ...
  it "reports a rule it cannot expand, rather than raising" do
    allow(IceCube::Rule).to receive(:from_ical).and_raise(ArgumentError, "bad rule")

    ics = calendar(events: [
                     event(rrule: "RRULE:FREQ=WEEKLY"),
                     event(dtstart: "DTSTART;TZID=Europe/Berlin:20260309T090000",
                           recurrence_id: "RECURRENCE-ID;TZID=Europe/Berlin:20260309T090000")
                   ])

    expect { expect(ics).to be_a_conforming_calendar }
      .to raise_error(RSpec::Expectations::ExpectationNotMetError, /could not be expanded: bad rule/)
  end

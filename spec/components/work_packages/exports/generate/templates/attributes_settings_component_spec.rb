...
RSpec.describe WorkPackages::Exports::Generate::Templates::AttributesSettingsComponent, type: :component do
  subject(:rendered_component) { render_inline(described_class.new(settings:, disabled:)) }

  let(:settings) do
    { footer_text: "Some Project", page_orientation: "portrait", hyphenation: false, hyphenation_language: "en" }
  end
  let(:disabled) { false }

  it "renders the footer_text field pre-filled with the given value" do
    expect(rendered_component).to have_field "footer_text", with: "Some Project"
  end

  it "renders the page_orientation select with the given value selected" do
    expect(rendered_component).to have_select "page_orientation", selected: "Portrait"
  end
  ...
  context "when page_orientation is landscape" do
    let(:settings) { { footer_text: "Some Project", page_orientation: "landscape" } }

    it "selects the landscape option" do
      expect(rendered_component).to have_select "page_orientation", selected: "Landscape"
    end
  end
  ...
  context "when settings is empty" do
    ...
    it "renders the footer_text field empty and hyphenation unchecked" do
      expect(rendered_component).to have_field "footer_text", with: ""
      expect(rendered_component).to have_unchecked_field "Hyphenation"
    end
  end
  ...
  context "when disabled" do
    ...
    it "renders every field disabled" do
      expect(rendered_component).to have_field "footer_text", disabled: true
      expect(rendered_component).to have_select "page_orientation", disabled: true

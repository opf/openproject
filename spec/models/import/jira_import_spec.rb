  ...
  describe "#find_op_leg" do
    ...
    context "when reference exists" do
      ...
      it "returns the OpenProject entity" do
        expect(jira_import.find_op_leg(jira_version)).to eq(version)
      end
    end

    context "when reference does not exist" do
      it "returns nil" do
        expect(jira_import.find_op_leg(jira_version)).to be_nil
      end
    end

    context "when reference exists in a different import" do
      let(:other_import) { create(:jira_import, jira:, author:) }

      before do
        create(:jira_open_project_reference,
               jira_import: other_import,
               jira_entity_class: jira_version.class.to_s,
               jira_entity_id: jira_version.id.to_s,
               op_entity_class: version.class.to_s,
               op_entity_id: version.id.to_s)
  ...
  describe "#find_jira_leg" do
    ...
    context "when reference exists" do
      ...
      it "returns the Jira entity" do
        expect(jira_import.find_jira_leg(version)).to eq(jira_version)
      end
    end

    context "when reference does not exist" do
      it "returns nil" do
        expect(jira_import.find_jira_leg(version)).to be_nil
      end
    end

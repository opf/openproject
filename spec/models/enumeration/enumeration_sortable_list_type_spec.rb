...
RSpec.describe Enumeration, "sortable list types" do
  it "gives every reorderable enumeration class its own list type", :aggregate_failures do
    expect(IssuePriority::SORTABLE_LIST_TYPE).to eq("issue_priority")
    expect(TimeEntryActivity::SORTABLE_LIST_TYPE).to eq("time_entry_activity")
    expect(DocumentType::SORTABLE_LIST_TYPE).to eq("document_type")
  end

  it "keeps the list types distinct across classes" do
    types = [IssuePriority, TimeEntryActivity, DocumentType].map { it::SORTABLE_LIST_TYPE }

    expect(types.uniq.size).to eq(3)
  end

  describe "anchor moves" do

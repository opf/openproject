...
RSpec.describe WorkPackage, "automations" do
  let(:work_package) do
  ...
  let(:conditions) do
    [Automations::Conditions::Status.new([status.id])]
  end

  let!(:automation) do
    action = build(:automation, :with_button_trigger)
  ...
  describe "#automations" do
    context "with the custom action having no restriction" do
      ...
      it "returns the action" do
        expect(work_package.automations(user))
          .to contain_exactly(automation)
      end
    end
    ...
    context "with a role restriction" do
      ...
      context "with the condition requiring a different role" do
        ...
        let(:conditions) do
          [Automations::Conditions::Role.new(other_role.id)]
        end

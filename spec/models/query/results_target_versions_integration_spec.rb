...
RSpec.describe Query::Results, "Grouping and sorting for target versions",
  ...
  let!(:alpha_wp) do
    create_wp_with_target_versions("Alpha wp", [alpha_version])
  end
  let!(:both_versions_wp) do
    create_wp_with_target_versions("Both versions wp", [beta_version, alpha_version])
  end
  let!(:beta_wp) do
    create_wp_with_target_versions("Beta wp", [beta_version])
  end
  ...
  let(:group_by) { nil }
  let(:sort_criteria) { [["target_versions", "asc"]] }
  ...
  def create_wp_with_target_versions(subject, versions, **attributes)
    create(:work_package, subject:, project:, version: versions.first, **attributes).tap do |wp|
      versions.drop(1).each do |version|
        create(:work_package_version, work_package: wp, version:)
      end
    end
  end
  ...
  describe "sorting ASC by target versions" do
    let(:sort_criteria) { [["target_versions", "asc"]] }
  ...
  describe "sorting DESC by target versions" do
    let(:sort_criteria) { [["target_versions", "desc"]] }
  ...
  describe "grouping by target versions" do
    let(:group_by) { "target_versions" }
    ...
    it "groups by the set of assigned versions" do
      ...
      # target version are grouped under the empty set.
    ...
    context "with sums displayed" do
      ...
      let!(:alpha_wp) do
        create_wp_with_target_versions("Alpha wp", [alpha_version], estimated_hours: 2)
      end
      let!(:both_versions_wp) do
        create_wp_with_target_versions("Both versions wp", [beta_version, alpha_version], estimated_hours: 3)
      end
  ...
  context "with equally named versions in different projects" do
    ...
    let!(:same_name_wp) do
      create(:work_package, subject: "Same name other project wp", project: other_project, version: same_name_version)
    end

...
describe ::Query::Results do
  ...
  describe '#work_packages' do
    ...
    let!(:role_pm) { FactoryGirl.create(:role, :name => 'Manager', :permissions => [:view_work_packages, :edit_work_packages, :create_work_packages, :delete_work_packages])}
    let!(:role_dev) { FactoryGirl.create(:role, :name => 'Developer', :permissions => [:view_work_packages])}
    let!(:user_1) { FactoryGirl.create(:user, :member_in_project => project_1,
                                       :member_through_role => [role_dev, role_pm]) }
    let!(:member) { FactoryGirl.create(:member,
                                       project: project_2,
                                       principal: user_1,
                                       roles: [role_pm]) }
    let!(:user_2) { FactoryGirl.create(:user, :member_in_project => project_2,
                                       :member_through_role => role_dev) }

    let!(:wp_p1) {(1..3).collect { FactoryGirl.create(:work_package, :project=> project_1, assigned_to_id: user_1.id) }}
    let!(:wp_p2) { FactoryGirl.create(:work_package, :project => project_2, assigned_to_id: user_2.id) }
    let!(:wp2_p2) { FactoryGirl.create(:work_package, :project => project_2, assigned_to_id: user_1.id) }
    ...
    context 'when a project is seted' do
      before do
    ...
    context 'when no project is seted' do
      before do

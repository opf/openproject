...
RSpec.describe Automation do
  let(:stubbed_instance) { build_stubbed(:automation, :with_button_trigger) }
  let(:instance) { create(:automation, :with_button_trigger, name: "zzzzzzzzz") }
  let(:other_instance) { create(:automation, :with_button_trigger, name: "aaaaa") }
  ...
  describe "validations" do
    ...
    it "is invalid without a trigger" do
      expect(build_stubbed(:automation))
        .not_to be_valid
    end

    it "is invalid with more than one button trigger" do
      automation = build(:automation, :with_button_trigger)
      automation.triggers << Automations::Triggers::Button.new(button_label: "Another button")
  ...
  describe ".actions" do
    ...
    it "can be persisted" do
      ...
      expect(described_class.find(instance.id).actions.map { |a| [a.key, a.values] })
        .to contain_exactly([:assigned_to, [1]])
    end
  end
  ...
  describe ".conditions" do
    ...
    it "can be persisted" do
      ...
      expect(described_class.find(instance.id).conditions.map { |a| [a.key, a.values] })
        .to contain_exactly([:status, [status.id]], [:role, [role.id]])
    end
  ...
  describe "#conditions_fulfilled?" do
    let(:work_package) { instance_double(WorkPackage) }
    let(:user) { instance_double(User) }

    let(:stubbed_condition1) do
      condition = instance_double(Automations::Conditions::Base)
      allow(condition)
    ...
    context "with all conditions fulfilled" do
      it "is true" do
        expect(stubbed_instance)
    ...
    context "with one condition not fulfilled" do

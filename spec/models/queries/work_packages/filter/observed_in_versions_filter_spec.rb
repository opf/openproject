...
RSpec.describe Queries::WorkPackages::Filter::ObservedInVersionsFilter do
  ...
  it_behaves_like "basic query filter" do
    ...
    # Unlike the target versions filter, this one has no deprecated
    # single-valued counterpart to alternate with, so it is offered regardless
    # of the multiple versions feature.
    ...
    describe "#where" do
      ...
      # The mirror of the target filter's guard: a target row on the same
      # version must never be mistaken for an observed one.
      ...
      context 'for "!" with a version' do
        ...
        it "returns work packages not observed in that version, including ones without observed versions" do
          expect(result).to contain_exactly(wp_observing_closed, wp_observing_locked, wp_targeting_only,
                                            wp_without_versions)
        end
      end

      context 'for "*" (any observed version)' do
        let(:operator) { "*" }
        let(:values) { [] }

        it "returns work packages with at least one observed version" do
          expect(result).to contain_exactly(wp_observing_open, wp_observing_closed, wp_observing_locked)
        end
      end

      context 'for "!*" (no observed version)' do
        let(:operator) { "!*" }
        let(:values) { [] }

        it "returns work packages without any observed version" do

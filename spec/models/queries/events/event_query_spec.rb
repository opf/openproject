...
describe Queries::Events::EventQuery, type: :model do
  ...
  context 'with a read_ian filter' do
    ...
    describe '#results' do
      it 'is the same as handwriting the query' do
        expected = base_scope.where("events.read_ian IN ('t')").order(id: :desc)
  ...
  context 'with a non existent filter' do
    ...
    describe '#results' do
      it 'returns a query not returning anything' do
        expected = Event.where(Arel::Nodes::Equality.new(1, 0))
  ...
  context 'with an id sortation' do
    ...
    describe '#results' do
      it 'is the same as handwriting the query' do
        expected = base_scope.merge(Event.order(id: :asc))
  ...
  context 'with a read_ian sortation' do
    ...
    describe '#results' do
      it 'is the same as handwriting the query' do
        expected = "SELECT \"events\".* FROM \"events\" WHERE \"events\".\"recipient_id\" = #{recipient.id} ORDER BY \"read_ian\" DESC, \"events\".\"id\" DESC"
  ...
  context 'with a reason sortation' do
    ...
    describe '#results' do
      it 'is the same as handwriting the query' do
        expected = "SELECT \"events\".* FROM \"events\" WHERE \"events\".\"recipient_id\" = #{recipient.id} ORDER BY \"reason\" DESC, \"events\".\"id\" DESC"

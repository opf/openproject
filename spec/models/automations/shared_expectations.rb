...
RSpec.shared_context "with an automation action" do
  let(:instance) do
    described_class.new
  end
  let(:expected_key) do
...
RSpec.shared_examples_for "base custom action" do
  include_context "with an automation action"
...
RSpec.shared_examples_for "associated custom action" do
  include_context "with an automation action" do
    describe "#apply" do
      let(:work_package) { build_stubbed(:work_package) }

      it "sets the associated_id in the work package to the action's value" do
        allow(work_package).to receive(:"#{key}_id=")
...
RSpec.shared_examples_for "date custom action validations" do
  describe "#validate" do
    ...
    it "adds an error on actions if there are more values than one (depending on multi_value?)" do
      instance.values = [Time.zone.today + 4.days, Time.zone.today - 5.days]
...
RSpec.shared_examples_for "date values transformation" do
  describe "#values" do
    it "transforms the values to integers" do
      instance.values = ["2015-03-29", Time.zone.today, nil, (Time.zone.today - 1.day).to_datetime, "bogus", "%CURRENT_DATE%"]

      expect(instance.values)
        .to contain_exactly(Date.parse("2015-03-29"), Time.zone.today, nil, Time.zone.today - 1.day, "%CURRENT_DATE%")
    end
  end
end
...
RSpec.shared_examples_for "date custom action apply" do
  describe "#apply" do
    ...
    it "sets the daate to the action's value" do
      instance.values = [Time.zone.today + 5.days]

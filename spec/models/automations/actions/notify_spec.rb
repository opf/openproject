...
RSpec.describe Automations::Actions::Notify do
  ...
  let(:allowed_values) do
    ...
    allow(Principal).to receive_messages(not_locked: Principal, select: Principal,
                                         select_for_name: Principal, ordered_by_name: users)
  ...
  it_behaves_like "base custom action" do
    ...
    describe "#apply" do
      ...
      it "adds a note with all values distinguished by type" do
        ...
        allow(Principal).to receive_messages(not_locked: Principal, select: Principal,
                                             select_for_name: Principal, ordered_by_name: Principal,
                                             where: principals)

        instance.values = principals.map(&:id)

        allow(work_package).to receive(:journal_notes=)

        instance.apply(work_package)

        expect(work_package)
          .to have_received(:journal_notes=)

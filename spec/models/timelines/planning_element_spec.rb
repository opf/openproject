...
describe Timelines::PlanningElement do
  ...
  describe '- Validations ' do
    ...
    describe 'end_date' do
      ...
      it 'is invalid if planning_element is milestone and end_date is not on start_date' do
        attributes[:planning_element_type] = FactoryGirl.build(:timelines_planning_element_type, :is_milestone => true)
        attributes[:start_date]            = Date.today
        attributes[:end_date]              = Date.today + 1.week
        planning_element = Timelines::PlanningElement.new.tap { |pe| pe.send(:assign_attributes, attributes, :without_protection => true) }
    ...
    describe 'parent' do
      it 'is invalid if parent is_milestone' do
        parent = Timelines::PlanningElement.new.tap do |pe|
          pe.send(:assign_attributes, attributes.merge(:planning_element_type => FactoryGirl.build(:timelines_planning_element_type, :is_milestone => true)), :without_protection => true)
        end
  ...
  describe 'derived attributes' do
    before do
      @pe1  = FactoryGirl.create(:timelines_planning_element, :project_id => project.id)
      @pe11 = FactoryGirl.create(:timelines_planning_element, :project_id => project.id, :parent_id => @pe1.id)
      @pe12 = FactoryGirl.create(:timelines_planning_element, :project_id => project.id, :parent_id => @pe1.id)
    end
  ...
  describe 'alternate dates' do
    describe 'auto-creation' do
      ...
      it 'creates an alternate date whenever a planning element is created' do
        pe = Timelines::PlanningElement.new.tap { |pe| pe.send(:assign_attributes, attributes, :without_protection => true) }
        pe.save
    ...
    describe 'at_time scope' do
      ...
      before do
        ...
        pe = FactoryGirl.create(:timelines_planning_element,
        ...
        changes.each do |change|
          FactoryGirl.create(:timelines_alternate_date,
      ...
      it 'returns the current data when timestamp is after last edit' do
        fetched = Timelines::PlanningElement.at_time(changes.last[:time] + 1.day).find(@pe.id)
      ...
      it 'returns nothing when timestamp is before first edit' do
        expect {
          Timelines::PlanningElement.at_time(changes.first[:time] - 1.day).find(@pe.id)
        }.to raise_error(ActiveRecord::RecordNotFound)
      end

      it 'returns intial data when timestamp is between first and second edit' do
        edit = changes.first
        fetched = Timelines::PlanningElement.at_time(edit[:time] + 1.hour).find(@pe.id)
  ...
  describe 'scenarios' do
    ...
    describe 'w/ scenarios' do
      ...
      describe 'w/ alternate dates' do
        let(:alternate_date) { FactoryGirl.create(:timelines_alternate_date,
  ...
  describe 'journal' do
    ...
    let(:pe_type)     { FactoryGirl.create(:timelines_planning_element_type) }
    ...
    describe "a planning elements' journals includes changes to associated alternate dates start/end date" do
      let(:journal_planning_element) { FactoryGirl.create(:timelines_planning_element) }
      let(:journal_scenario) { FactoryGirl.create(:timelines_scenario) }
  ...
  describe 'acts as paranoid destroy' do
    before(:each) do
      @pe1 = FactoryGirl.create(:timelines_planning_element,
                            :project_id => project.id,
    ...
    it 'should set deleted_at on destroy' do
      ...
      expect { Timelines::PlanningElement.without_deleted.find(@pe1.id) }.to raise_error(ActiveRecord::RecordNotFound)
      Timelines::PlanningElement.find(@pe1.id).should_not be_nil
    end
    ...
    it 'should keep associated objects' do
      ...
      pe11  = FactoryGirl.create(:timelines_planning_element,
                             :project_id => project.id,
                             :parent_id  => @pe1.id,
                             :start_date => Date.new(2011, 1, 1),
                             :end_date   => Date.new(2011, 2, 1))
    ...
    it 'should adjust parent start and end dates' do
        ...
        pe11  = FactoryGirl.create(:timelines_planning_element,
                               ...
                               :start_date => Date.new(2011, 1, 1),
                               :end_date   => Date.new(2011, 2, 1))
        pe12  = FactoryGirl.create(:timelines_planning_element,
                               :project_id => project.id,
                               :parent_id  => @pe1.id,
                               :start_date => Date.new(2012, 2, 1),
                               :end_date   => Date.new(2012, 6, 1))
        pe13  = FactoryGirl.create(:timelines_planning_element,
                               :project_id => project.id,
                               :parent_id  => @pe1.id,
    ...
    it 'should restore elements without touching the children' do
      ...
      @pe1 = Timelines::PlanningElement.find(@pe1.id)
      @pe1.restore!
    ...
    it 'moves all child elements to trash' do
      ...
      pe11  = FactoryGirl.create(:timelines_planning_element, :project_id => project.id, :parent_id => pe1.id)
      pe12  = FactoryGirl.create(:timelines_planning_element, :project_id => project.id, :parent_id => pe1.id)
      pe121 = FactoryGirl.create(:timelines_planning_element, :project_id => project.id, :parent_id => pe12.id)
      pe2   = FactoryGirl.create(:timelines_planning_element, :project_id => project.id)
      ...
      [pe1, pe11, pe12, pe121].each do |pe|
        Timelines::PlanningElement.without_deleted.find_by_id(pe.id).should be_nil
        Timelines::PlanningElement.find_by_id(pe.id).should_not be_nil
      end

      Timelines::PlanningElement.without_deleted.find_by_id(pe2.id).should == pe2
    end

    it 'should delete the object permanantly when using destroy!' do
      @pe1.destroy!

      Timelines::PlanningElement.without_deleted.find_by_id(@pe1.id).should be_nil
      Timelines::PlanningElement.find_by_id(@pe1.id).should be_nil
    end
    ...
    it 'destroys all child elements' do
      ...
      [pe1, pe11, pe12, pe121].each do |pe|
        Timelines::PlanningElement.without_deleted.find_by_id(pe.id).should be_nil
        Timelines::PlanningElement.find_by_id(pe.id).should be_nil
      end

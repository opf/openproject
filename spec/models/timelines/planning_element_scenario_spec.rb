...
describe Timelines::PlanningElementScenario do
  let(:project)          { FactoryGirl.build(:project) }
  let(:planning_element) { FactoryGirl.build(:timelines_planning_element, :project          => project) }
  let(:scenario)         { FactoryGirl.build(:timelines_scenario,         :project          => project) }
  let(:alternate_date)   { FactoryGirl.build(:timelines_alternate_date,   :scenario         => scenario,
                                                                      :planning_element => planning_element) }

  let(:subject) { Timelines::PlanningElementScenario.new(alternate_date) }

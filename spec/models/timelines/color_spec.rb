  ...
  describe '- Validations ' do
    ...
    describe 'name' do
      it 'is invalid w/o a name' do
        attributes[:name] = nil
        color = Timelines::Color.new(attributes)
    ...
    describe 'hexcode' do
      ...
      it 'is invalid w/ malformed hexcodes' do
        Timelines::Color.new(attributes.merge(:hexcode => '0#FFFFFF')).should_not be_valid
        Timelines::Color.new(attributes.merge(:hexcode => '#FFFFFF0')).should_not be_valid
        Timelines::Color.new(attributes.merge(:hexcode => 'white')).   should_not be_valid
      end

      it 'fixes some wrong formats of hexcode automatically' do
        color = Timelines::Color.new(attributes.merge(:hexcode => 'FFCC33'))
        ...
        color = Timelines::Color.new(attributes.merge(:hexcode => 'fc3'))
        color.should be_valid
        color.hexcode.should == '#FFCC33'
      ...
      it 'is valid w/ proper hexcodes' do
        Timelines::Color.new(attributes.merge(:hexcode => '#FFFFFF')). should be_valid
        Timelines::Color.new(attributes.merge(:hexcode => '#FF00FF')).should be_valid
      end
    end
  end
end

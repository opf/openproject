...
RSpec.describe UserNonWorkingTimes::DeleteContract do
  include_context "ModelContract shared context"

  let(:target_user) { build_stubbed(:user) }
  let(:current_user) { build_stubbed(:user) }
  let(:non_working_day) { build_stubbed(:user_non_working_time, user: target_user) }
  let(:contract) { described_class.new(non_working_day, current_user) }

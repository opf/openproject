...
describe Mails::MemberCreatedJob, type: :model do
  ...
  before do
    %i[added_project updated_global].each do |mails|
      allow(MemberMailer)
        .to receive(mails)
  ...
  context 'with a group membership' do
    ...
    context 'with the user not having had a membership before the group`s membership was added' do
      let(:member_role_inherited_from) { group.id }
    ...
    context 'with the user having had a membership before the group`s membership was added' do

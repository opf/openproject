...
class CreateLlmModels < ActiveRecord::Migration[8.1]
  def change
    ...
    # A model the server stops reporting keeps its row here, so a default can be
    # a reference rather than an identifier that nothing resolves. Clearing it on
    # delete is what the administrator is warned about before removing a model.

class PermissionMaterializedView < ActiveRecord::Migration[7.0]
  ...
  FROM
          (
SELECT * from roles WHERE roles.builtin IN (#{Role::BUILTIN_NON_MEMBER}, #{Role::BUILTIN_ANONYMOUS})
) roles
		  LEFT JOIN
		    role_permissions
		  ...
		  LEFT JOIN
		    enabled_modules
			ON
			  (enabled_modules.name = permission_module_map.project_module)
		  WHERE
		  	(permission_module_map.project_module IS NULL) OR (permission_module_map.project_module IS NOT NULL AND (enabled_modules.name = permission_module_map.project_module))
			GROUP BY
	  permission_module_map.permission,
	  roles.id,
	  project_id
